...
```bash
claude --plugin-dir plugins/swarmlo-migrations
```
...
- **Verification:** `bash plugins/swarmlo-migrations/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `migrations` AgentDB namespace (kebab-case is implicit when the plugin name is the intent — same documented exception as `federation`). Follows the convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/swarmlo-migrations/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmlo-migrations plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-migrations-contract.md)

## Related Plugins

- `swarmlo-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `swarmlo-adr` -- Document schema change decisions as ADRs
- `swarmlo-ddd` -- Align migration boundaries with aggregate roots
- `swarmlo-observability` -- Track migration execution duration and failure rates
