...
```bash
claude --plugin-dir plugins/swarmlo-sparc
```
...
- **Verification:** `bash plugins/swarmlo-sparc/scripts/smoke.sh` is the contract.
...
|-------|-------|-----------------|
| **Specification** | [swarmlo-goals](../swarmlo-goals/docs/adrs/0001-goals-contract.md) (deep-research) | Multi-source research orchestration to gather requirements |
| **Pseudocode** | `swarmlo-sparc` (this plugin) | Pseudocode generation + complexity annotation |
| **Architecture** | [swarmlo-adr](../swarmlo-adr/docs/adrs/0001-adr-plugin-pattern.md) + [swarmlo-ddd](../swarmlo-ddd/docs/adrs/0001-ddd-contract.md) | ADR creation + bounded-context modeling |
| **Refinement** | [swarmlo-jujutsu](../swarmlo-jujutsu/docs/adrs/0001-jujutsu-contract.md) + swarmlo-testgen | Diff-aware refactor + test gap analysis |
...
This plugin owns three AgentDB namespaces, all kebab-case compliant per [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md):
...
- [`ADR-0001` — swarmlo-sparc plugin contract (phase-to-plugin alignment, namespace coordination, smoke as contract)](./docs/adrs/0001-sparc-contract.md)

## Related Plugins

- `swarmlo-agentdb` — namespace convention owner; backing store for sparc-state/-phases/-gates
- `swarmlo-goals` — Specification phase deep-research
- `swarmlo-adr` -- Architecture decisions recorded as ADRs in Phase 3
- `swarmlo-ddd` -- Architecture phase uses DDD bounded context patterns
- `swarmlo-jujutsu` -- Refinement phase diff analysis
- `swarmlo-docs` -- Completion phase documentation generation
