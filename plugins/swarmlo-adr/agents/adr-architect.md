...
- `mcp__plugin_swarmlo-core_swarmlo__agentdb_hierarchical-store` / `agentdb_hierarchical-query` — ADR tree storage.
- `mcp__plugin_swarmlo-core_swarmlo__agentdb_causal-edge` / `agentdb_causal-query` — relationship tracking.
- `mcp__plugin_swarmlo-core_swarmlo__memory_store` / `memory_search` — semantic search.
...
- **swarmlo-jujutsu**: Use diff analysis on PRs to check ADR compliance before merge.
- **swarmlo-docs**: Trigger doc generation when ADRs change status.
