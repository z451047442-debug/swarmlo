...
argument-hint: "<title>"
allowed-tools: mcp__plugin_swarmlo-core_swarmlo__agentdb_hierarchical-store mcp__plugin_swarmlo-core_swarmlo__agentdb_hierarchical-query mcp__plugin_swarmlo-core_swarmlo__agentdb_causal-edge mcp__plugin_swarmlo-core_swarmlo__memory_store mcp__plugin_swarmlo-core_swarmlo__memory_search Bash Read Write Edit Grep Glob
---
...
4. **Store in AgentDB** -- Call `mcp__plugin_swarmlo-core_swarmlo__agentdb_hierarchical-store` with:
   - key: `mem:ADR-NNN`
   - tier: `semantic`
   - value: a JSON-encoded string: `{ "id": "ADR-NNN", "title": "<title>", "status": "proposed", "date": "<today>", "file": "docs/adr/ADR-NNN-<slug>.md" }`

5. **Find related ADRs** -- Call `mcp__plugin_swarmlo-core_swarmlo__memory_search` with the title as query in namespace `adr-patterns` to find related decisions. If matches are found, add them to the Links section and call `mcp__plugin_swarmlo-core_swarmlo__agentdb_causal-edge` for each relationship with:
   - sourceId: `mem:ADR-NNN`
   - targetId: `mem:ADR-RELATED`
   - relation: `depends-on`

6. **Store pattern** -- Call `mcp__plugin_swarmlo-core_swarmlo__memory_store` in namespace `adr-patterns` with key `ADR-NNN` and the title + context as value for future semantic search.
