...
[[ -f "$ROOT/commands/swarmlo-loop.md" ]] || miss="$miss missing-loop-cmd"
[[ -f "$ROOT/commands/swarmlo-schedule.md" ]] || miss="$miss missing-schedule-cmd"
[[ -z "$miss" ]] && ok || bad "$miss"
...
step "9. swarmlo-autopilot 270s heartbeat contract cross-reference"
F="$ROOT/README.md"
grep -q "swarmlo-autopilot" "$F" \
...
grep -q "swarmlo-docs" "$F" || miss="$miss docs-attribution"
grep -q "swarmlo-testgen" "$F" || miss="$miss testgen-attribution"
[[ -z "$miss" ]] && ok || bad "$miss"
