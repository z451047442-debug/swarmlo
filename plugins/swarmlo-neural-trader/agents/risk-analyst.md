...
- **swarmlo-observability**: Real-time risk dashboards and alerting
- **swarmlo-cost-tracker**: PnL tracking and fee attribution
- **swarmlo-agentdb**: Historical risk event storage for pattern matching
...
Message schemas: `SignalProposal`, `RiskDecision` in `plugins/swarmlo-neural-trader/src/pipeline-messages.ts`.
