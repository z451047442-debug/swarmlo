---
name: harness-mint
description: Scaffold a custom AI agent harness via `metaharness new <name> --template <id> --host <id>`. Defaults to DRY-RUN (no writes) unless --confirm is passed. Refuses to write to the calling repo root or anywhere inside it. Honors ADR-150 architectural constraint + swarmlo's "destructive-action confirmation" pattern.
...
- host: claude-code
- target: /tmp/swarmlo-mint-1718560000-my-harness
- confirm: false
- willWrite: false
...
Swarmlo's behavioral rules say "executing actions with care" — destructive
