...
Example outputs from the `swarmlo-goals` plugin's `dossier-collect` skill (ADR-099). Each subdirectory contains:
...
| [swarmlo-goals/](./swarmlo-goals/) | `swarmlo-goals` | concept (plugin) | Glob, Read, Grep |

## How to generate your own

```
/swarmlo-goals:dossier-collect <seed> [--max-depth N] [--max-breadth N] [--budget-usd N]
```
