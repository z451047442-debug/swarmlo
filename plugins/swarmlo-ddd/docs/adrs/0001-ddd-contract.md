...
`swarmlo-ddd` (v0.1.1) is a code-scaffolding plugin (no MCP tools of its own). Surface:
...
Stores the domain model as a navigable graph in AgentDB with hierarchical nodes + causal edges for context dependencies. Pairs naturally with `swarmlo-adr` (domain decisions) and `swarmlo-sparc` (Architecture phase).
...
2. README augment: Compatibility (pin v3.6), Namespace coordination (claims `ddd-patterns`; defers to swarmlo-agentdb ADR-0001), Verification + Architecture Decisions sections.
