# Swarmlo v3.32.35: Adaptive Swarms and Safer Learning Loops

Swarmlo v3.32.35 gives long-running agent teams a controlled way to learn which
...
remove stale vectors; and daemon auto-start is limited to actual Swarmlo
projects.

## Install or upgrade

```bash
npm install --global swarmlo@3.32.35
swarmlo doctor
```
...
swarmlo swarm pheromone
swarmlo agent metrics --format json
```
...
Live mode changes Swarmlo dispatch eligibility only. It does not terminate
...
```json
{
  "schemaVersion": "swarmlo.flywheel-anchor-manifest/v1",
  "path": "my-project-anchor.json",
  "sha256": "sha256:<canonical-task-hash>"
}
```

The referenced anchor contains at least four labelled tasks. Swarmlo binds its
canonical hash into new evaluation receipts. A non-Swarmlo repository without a
project anchor now fails closed instead of silently optimizing against Swarmlo's
...
These are Swarmlo benchmark results for that workload, not a universal production
performance claim.
...
This release ships [#2848](https://github.com/z451047442-debug/swarmlo/pull/2848) and
resolves
[#2815](https://github.com/z451047442-debug/swarmlo/issues/2815),
[#2819](https://github.com/z451047442-debug/swarmlo/issues/2819),
[#2832](https://github.com/z451047442-debug/swarmlo/issues/2832),
[#2834](https://github.com/z451047442-debug/swarmlo/issues/2834),
[#2839](https://github.com/z451047442-debug/swarmlo/issues/2839), and
[#2840](https://github.com/z451047442-debug/swarmlo/issues/2840).
