# Security Audit — swarmlo-neural-trader (2026-05-20)
...
```
node scripts/audit-supply-chain.mjs 2>&1 | grep -A5 'swarmlo-neural-trader\|plugins/swarmlo-neural-trader' || echo "no plugin-specific findings"
```
...
grep -rn 'process.env\|API_KEY\|SECRET' \
  plugins/swarmlo-neural-trader/skills/ \
  plugins/swarmlo-neural-trader/src/
```
...
| src/sublinear-adapter.mjs:22, 34-35             | `SWARMLO_SUBLINEAR_NATIVE`    | DOCUMENTED — feature flag, no value |
| src/sublinear-adapter.ts:32, 118, 136-137       | `SWARMLO_SUBLINEAR_NATIVE`    | DOCUMENTED — TS mirror of the above |
...
```
grep -rn 'eval\|new Function' plugins/swarmlo-neural-trader/ | grep -v '\.md:'
```
...
```
grep -rn 'spawn\|execSync\|spawnSync' plugins/swarmlo-neural-trader/
```
...
- AIDefence MCP tools to call: `aidefence_has_pii`, `aidefence_scan`,
  ...
  `swarmlo-federation` plugins already use).
...
- `plugins/swarmlo-federation/README.md` — existing AIDefence wiring (reference pattern)
- `plugins/swarmlo-browser/agents/browser-agent.md` — existing AIDefence gates (reference pattern)
