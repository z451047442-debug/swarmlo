 ...
 * after every turn. The fix: invoke `scripts/swarmlo-hook.sh` (prefers a locally-
 * installed `swarmlo`/`claude-flow` binary, falls back to `npx --prefer-offline`,
 ...
 *   2. If it invokes `npx` or `scripts/swarmlo-hook.sh`, it MUST be non-fatal:
...
for (const file of files) {
  ...
  for (const { event, matcher, idx, command, continueOnError } of collectCommands(parsed)) {
    ...
    if (usesNpx && !PREFER_OFFLINE_RE.test(command)) {
      violations.push({
        file: rel, where,
        reason: 'invokes `npx` without `--prefer-offline` — re-resolves the dist-tag and re-installs from cold cache on every hook fire (#1921). Use scripts/swarmlo-hook.sh, or add `--prefer-offline`.',
        command,
      });
      continue;
    }
