...
- `mcp__plugin_swarmlo-core_swarmlo__embeddings_generate` -- generate embeddings for entity descriptions
...
npx swarmlo-cli@3.39.1 neural train --pattern-type knowledge-graph --epochs 10
```
...
```bash
npx swarmlo-cli@3.39.1 memory store --namespace knowledge-graph --key "entity-ENTITY_NAME" --value "ENTITY_METADATA_JSON"
npx swarmlo-cli@3.39.1 memory store --namespace knowledge-graph --key "pattern-PATTERN_NAME" --value "GRAPH_PATTERN_JSON"
npx swarmlo-cli@3.39.1 memory search --query "entities related to authentication" --namespace knowledge-graph
```

### Related Plugins

- **swarmlo-agentdb**: Underlying storage for entities, relations, and causal edges via HNSW-indexed AgentDB
- **swarmlo-core**: Researcher agent uses pathfinder traversal for codebase exploration
- **swarmlo-ruvector**: HNSW indexing for fast semantic search across graph nodes
- **swarmlo-intelligence**: SONA neural patterns learn from graph traversal trajectories
