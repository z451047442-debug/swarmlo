#!/usr/bin/env node
/**
 * swarmlo-hook.cjs — cross-platform Windows shim for cost-tracker's Stop hook (#2132)
 *
 ...
 *
 * On Windows, `swarmlo init` writes a `.claude/settings.json` that overrides
 * the Stop hook to invoke this shim instead — `node swarmlo-hook.cjs` —
 ...
 *
 * Usage: node swarmlo-hook.cjs [hook-args ignored]
 */
