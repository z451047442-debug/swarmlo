/**
 * AGNTCY OTel semantic-convention span attributes — Swarmlo's runtime-owned
 * half (ADR-380 §5).
 *
 ...
 * Per ADR-380 §5, Swarmlo and the companion metaharness ADR-240 split
 ...
 *
 *   - Swarmlo (runtime, this file) owns the two attributes that only exist
 ...
 * those constants. This module only defines the two Swarmlo owns.
 *
 * Wire these attribute names through the existing `swarmlo-observability`
 ...
 * `swarmlo swarm join <namespace>` session). Only meaningful once SLIM
...
/**
 * All AGNTCY OTel span attributes Swarmlo defines and is responsible for
 ...
 */
export const AGNTCY_SWARMLO_OWNED_SPAN_ATTRS = [
  AGNTCY_SPAN_ATTR_COORDINATION_EPISODE,
  AGNTCY_SPAN_ATTR_AUTHORIZATION_DECISION,
] as const;

/** Union type of the span attribute names Swarmlo owns. */
export type AgntcySwarmloOwnedSpanAttr =
  (typeof AGNTCY_SWARMLO_OWNED_SPAN_ATTRS)[number];
