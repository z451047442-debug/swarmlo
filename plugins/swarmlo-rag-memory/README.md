...
npx swarmlo memory store --key "oauth-flow" --value "OAuth2 with pkce for SPAs, use refresh tokens" --namespace patterns

# Search for it later (even across projects!)
npx swarmlo recall "oauth single page app"

# Retrieve exact entry
npx swarmlo memory retrieve --key "oauth-flow" --namespace patterns
```
...
```bash
claude --plugin-dir plugins/swarmlo-rag-memory
```
...
Verify gate state with `swarmlo doctor -c encryption`. Off by default; flipping it on doesn't require a migration step (legacy plaintext bytes are sniffed on read; first write after enable rewrites the DB encrypted).
...
When `swarmlo-ruvector` is also loaded, rag-memory delegates to ruvector's backend for advanced features:
...
- **Verification:** `bash plugins/swarmlo-rag-memory/scripts/smoke.sh` is the contract.

## Namespace coordination — claude-memories consumer

This plugin is the **canonical user-facing consumer** of the `claude-memories` reserved namespace defined in [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md). The auto-import flow:

```
Claude Code SessionStart hook
  → memory_import_claude (MCP)
  → claude-memories namespace (reserved, swarmlo-agentdb owned)
...
- [`ADR-0001` — swarmlo-rag-memory plugin contract (claude-memories reserved-namespace consumer, smoke as contract)](./docs/adrs/0001-rag-memory-contract.md)

## Related Plugins

- `swarmlo-agentdb` — Full AgentDB controller bridge (15 `agentdb_*` MCP tools); namespace convention owner; owns the `claude-memories` reserved namespace
- `swarmlo-ruvector` — Advanced vector operations (FlashAttention-3, Graph RAG, hybrid search)
- `swarmlo-rvf` — Portable RVF memory format for cross-machine export/import
- `swarmlo-knowledge-graph` — Entity extraction and graph traversal over memory
