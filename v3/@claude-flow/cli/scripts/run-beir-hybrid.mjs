...
//   BGE_SPARSE=1 BGE_MODEL=Xenova/bge-m3 node .../run-beir-hybrid.mjs
//                                                              # + m3 native sparse as 3rd RRF system (ADR-382)
//
// ADR-087.
...
// ADR-382: opt-in bge-m3 native sparse as a third RRF system. Off by
// default so ADR-088 BM25 baselines stay the default measured path.
const BGE_SPARSE = process.env.BGE_SPARSE === '1';
const SPARSE_RRF_WEIGHT = Number(process.env.SPARSE_RRF_WEIGHT) || 0.3; // BAAI dense:sparse = 1:0.3
...
const DATASET_RRF_WEIGHTS = {
  arguana: { dense: 1.0, bm25: 0.2, sparse: 0.3 },  // iter 26: match nfcorpus/scifact recipe (aggressive dense)
  nfcorpus: { dense: 1.0, bm25: 0.0, sparse: 0.3 }, // iter 14: pure dense fusion (0.2→0.0) RRF with single system + minMax norm preserved
  scifact: { dense: 1.0, bm25: 0.05, sparse: 0.3 },  // darwin iter2: small bm25 tie-breaker on top of pure dense (0.0→0.05)
  // ADR-382: `sparse` is the bge-m3 native-sparse RRF weight, only active with BGE_SPARSE=1.
};
...
// ADR-382: bge-m3 native-sparse doc cache — {docId → [[token, weight], ...]}
// stored as token STRINGS (portable across tokenizer versions).
function sparseCachePath() {
  const safe = BGE_MODEL.replace(/\//g, '_');
  return join(CACHE_DIR, `${safe}.sparse.json`);
}
function loadSparseCache() {
  if (!existsSync(sparseCachePath())) return null;
  const raw = JSON.parse(readFileSync(sparseCachePath(), 'utf-8'));
  return new Map(Object.entries(raw).map(([id, pairs]) => [id, new Map(pairs)]));
}
async function buildSparseCache(corpus, spEmb) {
  const cache = new Map();
  for (let i = 0; i < corpus.length; i++) {
    const d = corpus[i];
    const vec = await spEmb.embed(`${d.title || ''} ${d.text || ''}`);
    cache.set(d._id, new Map([...spEmb.decode(vec).entries()]));
    if (i % 500 === 0) console.log(`  sparse cache ${i}/${corpus.length}`);
  }
  const obj = {};
  for (const [id, m] of cache) obj[id] = [...m.entries()];
  writeFileSync(sparseCachePath(), JSON.stringify(obj));
  return cache;
}
...
async function main() {
  ...
  console.log(`RRF weights: dense=${weights.dense} bm25=${weights.bm25}${BGE_SPARSE ? ` sparse=${weights.sparse ?? SPARSE_RRF_WEIGHT}` : ''} (iter 3: dataset-specific for ${dataset})`);
  ...
  // §1.5 — bge-m3 native sparse system (ADR-382, opt-in via BGE_SPARSE=1).
  let spEmb = null;
  let sparseDocVecs = null;
  let sparseDotByTokenFn = null;
  if (BGE_SPARSE) {
    const sparse = await import(join(CLI_ROOT, 'dist/src/memory/bge-sparse.js'));
    spEmb = await sparse.getBgeSparseEmbedder(BGE_MODEL);
    if (!spEmb) { console.error('Sparse embedder failed:', sparse.getSparseStatus()); process.exit(4); }
    sparseDotByTokenFn = sparse.sparseDotByToken;
    sparseDocVecs = loadSparseCache();
    if (!sparseDocVecs) {
      console.log('Building sparse doc cache (batch=1, may take a while)...');
      sparseDocVecs = await buildSparseCache(corpus, spEmb);
    }
    console.log(`Sparse: ${BGE_MODEL} — ${sparseDocVecs.size} doc vectors, RRF weight ${SPARSE_RRF_WEIGHT}`);
  }
  ...
  if (RERANK) {
    ...
    const ceFn = await ce.getCrossEncoder(); // ADR-383: default = bge-reranker-v2-m3
  ...
  for (const qid of evalQueryIds) {
    ...
    // §2.5 — bge-m3 native sparse scores (lexical-weight dot products).
    let sparseScored = null;
    if (spEmb && sparseDocVecs) {
      const qSparse = spEmb.decode(await spEmb.embed(qtext));
      sparseScored = new Array(sparseDocVecs.size);
      let si = 0;
      for (const [id, docVec] of sparseDocVecs) {
        sparseScored[si++] = { id, score: sparseDotByTokenFn(qSparse, docVec) };
      }
    ...
    const sparseTopK = sparseScored
      ? sparseScored.slice(0, Math.min(TOP_PER_SYSTEM, sparseScored.length)).filter((s) => s.score > 0)
      : null;
    ...
    if (sparseTopK) {
      const sparseScoresNorm = minMaxNorm(sparseTopK.map((s) => s.score));
      sparseTopK.forEach((d, i) => { d.scoreNorm = sparseScoresNorm[i]; });
    }

    const rrfScores = new Map();
    const weights = DATASET_RRF_WEIGHTS[dataset] || { dense: 1.0, bm25: 1.0, sparse: SPARSE_RRF_WEIGHT };  // iter 3: dataset-specific weights
    const sparseWeight = weights.sparse ?? SPARSE_RRF_WEIGHT;
    ...
    if (sparseTopK) {
      for (let r = 0; r < sparseTopK.length; r++) {
        const id = sparseTopK[r].id;
        rrfScores.set(id, (rrfScores.get(id) || 0) + sparseWeight / (RRF_K + r + 1));
      }
    }
  ...
  const summary = {
    ...
    dataset,
    pipeline: `BM25+BGE RRF${BGE_SPARSE ? '+m3-sparse' : ''}${RERANK ? '+CE-rerank' : ''}`,
    bgeModel: BGE_MODEL,
    sparse: BGE_SPARSE,
    sparseWeight: BGE_SPARSE ? (weights.sparse ?? SPARSE_RRF_WEIGHT) : 0,
    rrfK: RRF_K,
    rerank: RERANK,
    queries: n,
