...
**Status**: Proposed
**Date**: 2026-06-07
**Plugin**: swarmlo-arena

## Context

`swarmlo-arena` implements the first executable slice of the competitive-ruliology initiative
(Swarmlo ADR-147 competitive arena/tournament modes, ADR-148 co-evolution, ADR-150 visualization
...
Every tool returns the Swarmlo envelope `{ success: boolean; result?: T; error?: { message } }`
...
- TS code persists full artifacts via `RunStore` (`FileRunStore` → `.swarmlo/arena/<runId>.json`;
  `InMemoryRunStore` for tests). This is the local stand-in for RuVector ADR-197.
- AgentDB persistence is performed at the **command/agent layer** (Swarmlo convention): each tool
  result carries an `agentdb` payload `{ namespace:"arena", key, value, tags }` for
  `mcp__plugin_swarmlo-core_swarmlo__memory_store`, making runs semantically searchable.
...
- **Positive**: stable, typed contract; zero runtime deps beyond Zod; engine is pure and
  unit-tested; no Swarmlo-core changes; clean seam for the RuVector data layer and the dashboard.
...
- Swarmlo ADR-147 (arena/tournament), ADR-148 (co-evolution), ADR-149 (dashboard), ADR-150 (viz split)
