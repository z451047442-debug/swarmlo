...
export async function generateConfigToml(options: ExtendedConfigTomlOptions = {}): Promise<string> {
  ...
  lines.push('# Documentation: https://github.com/z451047442-debug/swarmlo');
  lines.push('# ');
  ...
  lines.push('# =============================================================================');
  lines.push('# Swarmlo Agentic Policy Engine (ADR-324)');
  lines.push('# =============================================================================');
  lines.push('');
  lines.push('[policy]');
  ...
  // condition was tautological.
  {
    ...
    const hasSwarmlo = mcpServers.some(s => s.name === 'swarmlo' || s.name === 'claude-flow');
    if (!hasSwarmlo) {
      lines.push(...generateMcpServer(getSwarmloMcpServerConfig()));
      lines.push('');
    }
...
export async function generateCIConfigToml(platform: NodeJS.Platform = process.platform): Promise<string> {
  const mcpCommand = renderMcpServerToml(getSwarmloMcpServerConfig(platform, 300)).join('\n');
  return `# =============================================================================
