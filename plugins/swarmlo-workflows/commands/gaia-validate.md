...
(default `~/.cache/swarmlo/gaia/results-latest.json`). Motivated by UC Berkeley
...
```bash
node plugins/swarmlo-workflows/scripts/gaia-audit.mjs \
  --results ~/.cache/swarmlo/gaia/results-latest.json \
...
`~/.cache/huggingface`, `~/.cache/swarmlo/gaia/dataset`). `dynamic-eval` is tuned
to pass swarmlo's own multi-line `gcloud secrets … execSync` calls (fixed-string
...
5. If `--audit` is set and a results file exists, run
   `plugins/swarmlo-workflows/scripts/gaia-audit.mjs` (ADR-167); treat a CRITICAL
