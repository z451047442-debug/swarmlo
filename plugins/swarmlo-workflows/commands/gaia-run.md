...
Run GAIA benchmark questions through the swarmlo agent loop.
...
2. **Load dataset** — downloads and caches the GAIA validation split from
   Hugging Face (`~/.cache/swarmlo/gaia/`).  Cached files are reused on
   subsequent runs.
...
6. **Write output** — results land in `~/.cache/swarmlo/gaia/results-<sha>.json`.
...
| swarmlo iter 23 | 20.8% | 53 Q, web_search restored |
| swarmlo iter 15 | 9.4% | 53 Q, broken web_search |
...
6. Store the run record in memory: `npx swarmlo-cli@3.39.1 memory store --namespace gaia-runs --key "run-$(date +%Y%m%d-%H%M)" --value "$SUMMARY"`
