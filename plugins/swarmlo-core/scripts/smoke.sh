#!/usr/bin/env bash
# Structural smoke test for swarmlo-core v0.2.6 (ADR-0001).
set -u
...
step "2. .mcp.json registers a 'swarmlo' MCP server"
F="$ROOT/.mcp.json"
if [[ -f "$F" ]] && grep -q '"swarmlo"' "$F" && grep -q '"command"' "$F"; then
  ok
else
  bad ".mcp.json missing or no swarmlo server registration"
fi
...
miss=""
for s in init-project swarmlo-doctor swarmlo-status discover-plugins witness; do
...
n=$(grep -oE 'swarmlo-[a-z-]+' "$F" | sort -u | wc -l | tr -d ' ')
if [[ $n -ge 25 ]]; then
  ok
else
  bad "expected ≥25 distinct swarmlo-* references, got $n"
fi
...
step "9. /swarmlo-status command invokes doctor + status"
F="$ROOT/commands/swarmlo-status.md"
if grep -q "doctor" "$F" && grep -q "status" "$F"; then
  ok
else
  bad "swarmlo-status command missing doctor/status invocation"
fi
