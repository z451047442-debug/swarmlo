# Swarmlo v3.32.29: Capability Brain — Corrected Stable Train

v3.32.29 is the corrective stable publication of the Capability Brain work
first preserved in the immutable
[v3.32.28 source release](https://github.com/z451047442-debug/swarmlo/releases/tag/v3.32.28).
The v3.32.28 npm train was not published: the attempted procedure targeted
internal packages with the wrong credential, and the intended federation
`1.0.0-alpha.17` coordinate was already occupied by an older artifact. The
corrected flow uses the existing `ruvnet` npm session and publishes only the
three public packages. Swarmlo did not move the existing tag or reuse the
immutable package coordinate.

This release advances `swarmlo`, `claude-flow`, and `@claude-flow/cli` to
...
[separate v3.32.29 gist](https://gist.github.com/ruvnet/934130093d333bde1a3f0f83d7d95115).

Swarmlo has grown into a large agentic runtime, but its old MCP guidance catalog
...
v3.32.29 publishes the Swarmlo Capability Brain: a live, machine-readable map of
...
Swarmlo which capabilities fit a task and automatically choose from tools that
are actually registered in the current process.

## What you can use it for

- Discover every MCP tool registered in your running Swarmlo server.
- Route a task to suitable tools, agents, skills, and a swarm topology.
- Find capabilities across `.claude`, `.agents`, installed packages, and Swarmlo
  plugins.
...
> Use Swarmlo `guidance_brain` to recommend the best workflow for implementing
...
5. Execute in isolated scopes; Swarmlo coordinates while the coding agent works.
...
Public npm package train:

- `swarmlo@3.32.29`
- `claude-flow@3.32.29`
- `@claude-flow/cli@3.32.29`

Bundled component versions (not part of the normal standalone npm publish):
...
- ADR-329: Swarmlo Capability Brain for MCP guidance
