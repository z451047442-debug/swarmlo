#!/usr/bin/env node
/**
 * swarmlo-hook.cjs — cross-platform Node.js port of swarmlo-hook.sh (#2132, #2721)
 *
 * The bash shim (swarmlo-hook.sh) works on Mac/Linux but fails outright on
 ...
 *
 * Usage: node swarmlo-hook.cjs <hook-subcommand>
...
function invokeHook(bin, binArgs, hookSubcommand, hookArgs, stdinData) {
  ...
  // creates for globally-installed bins (`swarmlo`, `claude-flow`, `npx`) —
  ...
  // A real global `swarmlo`/`claude-flow` install still goes through the
  ...
  // Test-only: SWARMLO_HOOK_DEBUG_STDOUT surfaces the invoked CLI's own
  ...
  const debug = process.env.SWARMLO_HOOK_DEBUG_STDOUT === '1';
  try {
...
/** Best-effort: try swarmlo, then claude-flow, then npx. Never throws. */
function invokeCli(hookSubcommand, hookArgs, stdinData) {
  ...
  const override = process.env.SWARMLO_HOOK_CLI_OVERRIDE;
  if (override) {
    const [bin, ...binArgs] = override.split(' ').filter(Boolean);
    invokeHook(bin, binArgs, hookSubcommand, hookArgs, stdinData);
    return;
  }
  if (commandExists('swarmlo')) {
    invokeHook('swarmlo', [], hookSubcommand, hookArgs, stdinData);
    return;
  }
  ...
  // SKIP npx when SWARMLO_HOOK_SKIP_NPX=1 — used by CI smokes that test the
...
function main() {
  ...
  // (matches swarmlo-hook.sh's generic `swarmlo hooks "$@"` passthrough).
