# RuVocal — Swarmlo Web UI

> Swarmlo's multi-model AI chat with built-in Model Context Protocol (MCP) tool calling. Live at [**flo.ruv.io**](https://flo.ruv.io/).
...
RuVocal is the SvelteKit web app that lets you chat with Qwen, Claude, Gemini, or OpenAI while [Swarmlo](https://github.com/z451047442-debug/swarmlo) invokes the same ~210 MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — all directly from chat. No install, no API key needed to try the hosted demo.

It started as a fork of the [HuggingFace chat-ui](https://github.com/huggingface/chat-ui) v0.20.0 and has been extended with a WASM-MCP integration layer, parallel tool execution, an in-browser tool gallery, and a "Swarmlo Capabilities" tour modal. See [ADR-033](../../docs/adr/ADR-033-RUVOCAL-WASM-MCP-INTEGRATION.md) for the architecture.

## What RuVocal adds on top of upstream chat-ui

| | |
|---|---|
| 🛠️ **~210 MCP tools, prefixed** | Five Swarmlo server groups (Core, Intelligence, Agents, Memory, DevTools) plus a 18-tool in-browser WASM gallery |
...
| 📘 **Swarmlo Capabilities modal** | Question-mark icon → multi-section tour: models, tools, architecture, shortcuts |
...
| 🦾 **ruvLLM ready** | Native support for [ruvLLM](https://github.com/ruvnet/RuVector/tree/main/examples/ruvLLM) — Swarmlo's self-improving local model layer |
...
# Curated Swarmlo model list (optional — defaults to /models from the base URL)
...
## MCP Tools (the Swarmlo difference)
...
```env
MCP_SERVERS=[
  {"name":"Swarmlo Core","url":"https://mcp-bridge-...run.app/mcp/core","transport":"sse"},
  {"name":"Swarmlo Intelligence","url":"https://mcp-bridge-...run.app/mcp/intelligence","transport":"sse"},
  {"name":"Swarmlo Agents","url":"https://mcp-bridge-...run.app/mcp/agents","transport":"sse"},
  {"name":"Swarmlo Memory","url":"https://mcp-bridge-...run.app/mcp/memory","transport":"sse"},
  {"name":"Swarmlo DevTools","url":"https://mcp-bridge-...run.app/mcp/devtools","transport":"sse"},
...
- **Capabilities modal** — `src/lib/components/SwarmloHelpModal.svelte`
...
- 🏠 **Parent project** — [Swarmlo](https://github.com/z451047442-debug/swarmlo)
...
- 📋 **Roadmap** — [issue #1689](https://github.com/z451047442-debug/swarmlo/issues/1689)
