 ...
 *
 * The bash shim (swarmlo-hook.sh) works on Mac/Linux but fails on native
 ...
 *
 * Mac/Linux continue to use swarmlo-hook.sh via the plugin hooks.json files
 * (unchanged). On Windows, swarmlo init writes a .claude/settings.json that
 ...
 *
 * Usage: node swarmlo-hook.cjs <hook-subcommand> [args...]
 *   e.g. node swarmlo-hook.cjs post-edit --file "x.ts" --train-patterns
 */
...
/** Build the argv for the swarmlo/claude-flow/npx invocation */
function buildArgs(subcommand, extraArgs) {
  // The `hooks` word is prepended here, matching swarmlo-hook.sh convention.
...
function main() {
  ...
  // Priority 1: locally installed swarmlo binary
  if (commandExists('swarmlo')) {
    invokeHook('swarmlo', [], hookArgs, stdinData);
    done();
  }
  ...
  //
  // SKIP this when SWARMLO_HOOK_SKIP_NPX=1 — used by CI smokes that test
