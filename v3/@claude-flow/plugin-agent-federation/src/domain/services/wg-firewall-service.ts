 ...
 *   - `pf` for macOS — `pfctl -a swarmlo-fed -f <file>`, anchor-scoped so
 *     swarmlo's rules don't collide with the operator's main pf ruleset.
 *
...
export interface WgFirewallServiceConfig {
  ...
  /** WG interface name to scope rules to. Defaults to `swarmlo-fed`. */
  readonly interfaceName?: string;
  /** Where the operator should write the rule file. Used for the load-command string. */
  readonly rulePath?: string;
  /** pf anchor (macOS only). Defaults to `swarmlo-fed`. */
...
const DEFAULT_INTERFACE = 'swarmlo-fed';
const DEFAULT_PF_ANCHOR = 'swarmlo-fed';
...
export class WgFirewallService {
  ...
  private renderNftables(projections: ReadonlyArray<{
    ...
    lines.push('# Generated by swarmlo federation plugin — ADR-111 Phase 4.');
    ...
    lines.push('');
    lines.push('table inet swarmlo_fed {');
  ...
  private defaultRulePath(): string {
    return this.platform === 'linux-nftables'
      ? '/etc/nftables.d/swarmlo-fed.nft'
