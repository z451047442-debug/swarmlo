...
```bash
npx swarmlo-cli@latest memory store --namespace llm-configs --key "config-PROVIDER-MODEL" --value "PARAMS_AND_RESULTS"
npx swarmlo-cli@latest memory search --query "config for PROVIDER" --namespace llm-configs
```
