...
/**
 * Combine dense and bge-m3 native-sparse scores (ADR-382). Both vectors
 * must be aligned by docIndex. Each side is min-max normalised first, then
 * combined as denseWeight * dense + sparseWeight * sparse. Defaults follow
 * BAAI's published dense:sparse = 1:0.3 fusion recipe.
 */
export function fuseDenseSparse(
  dense: number[],
  sparse: number[],
  denseWeight = 1.0,
  sparseWeight = 0.3,
  ...
  if (dense.length !== sparse.length) {
    throw new Error('fuseDenseSparse: dense and sparse length mismatch');
  }
  const dN = normalise(dense);
  const sN = normalise(sparse);
  return dN.map((d, i) => denseWeight * d + sparseWeight * sN[i]);
}
