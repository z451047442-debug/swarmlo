...
`swarmlo-adr` is the meta-plugin that manages ADR lifecycle for projects (this one included — every other plugin in this repo is documenting itself with an ADR-0001 by the very contract this plugin proposes). The plugin already shipped four useful pieces:
...
Gaps observed against the pattern established by `swarmlo-ruvector` ADR-0001, `swarmlo-agentdb` ADR-0001, `swarmlo-browser` ADR-0001, `swarmlo-intelligence` ADR-0001:
...
3. **Free-form namespace usage.** The agent + skills write to `adr-patterns` (kebab-case, follows the convention) but never cite the convention from `swarmlo-agentdb` ADR-0001.
...
- **Namespace coordination** — explicit deferral to `swarmlo-agentdb` ADR-0001 §"Namespace convention". `adr-patterns` is the canonical namespace this plugin owns; the reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
7. README has Namespace coordination section deferring to `swarmlo-agentdb` ADR-0001.
...
```bash
bash plugins/swarmlo-adr/scripts/smoke.sh
...
- `plugins/swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention this plugin defers to
- `plugins/swarmlo-browser/docs/adrs/0001-browser-skills-architecture.md` — session-as-skill precedent
- `plugins/swarmlo-intelligence/docs/adrs/0001-intelligence-surface-completeness.md` — surface-completeness precedent
- `plugins/swarmlo-adr/REFERENCE.md` — heavy reference data (template, AgentDB paths) per ADR-098 Part 2 token-diet
