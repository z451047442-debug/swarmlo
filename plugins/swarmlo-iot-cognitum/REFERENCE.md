# swarmlo-iot-cognitum — Operations Reference
...
Workers are dispatched by the host daemon when `swarmlo-iot-cognitum` is loaded. Verify via `swarmlo hooks worker list` and `swarmlo hooks worker status`.
