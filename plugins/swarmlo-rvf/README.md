...
/plugin install swarmlo-rvf@swarmlo
```
...
## Encryption at rest (swarmlo 3.6.25+)
...
- **Off by default** (`CLAUDE_FLOW_ENCRYPT_AT_REST` unset / falsy) — sessions are plaintext JSON at mode 0600, same as swarmlo 3.6.24 and earlier.
...
Confirm the gate state with `swarmlo doctor -c encryption`.
...
- **Verification:** `bash plugins/swarmlo-rvf/scripts/smoke.sh` is the contract.
...
|-------|-------|-------------|
| **Portable memory + session persistence** | `swarmlo-rvf` (this plugin) | High-level skills for save/restore, cross-machine transfer |
| **Browser sessions as RVF** | [swarmlo-browser ADR-0001](../swarmlo-browser/docs/adrs/0001-browser-skills-architecture.md) | Each browser session is allocated as an RVF container at session-start (manifest, trajectory, screenshots, snapshots, cookies, findings) |
| **RVF tooling (10 subcommands)** | [swarmlo-ruvector ADR-0001](../swarmlo-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md) | `ruvector rvf create|ingest|query|status|segments|derive|compact|export|examples|download` |

This plugin sits on top of ruvector's tooling and feeds browser's session-as-RVF model.

## Namespace coordination

This plugin owns the `rvf-sessions` AgentDB namespace (kebab-case, follows the convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/swarmlo-rvf/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmlo-rvf plugin contract (cross-plugin RVF ownership table, namespace coordination, smoke as contract)](./docs/adrs/0001-rvf-contract.md)

## Related Plugins

- `swarmlo-ruvector` — exposes the `ruvector rvf *` tooling this plugin sits on top of
- `swarmlo-browser` — uses RVF containers for session-as-skill artifacts (ADR-0001 there)
