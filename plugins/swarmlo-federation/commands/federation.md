...
**init**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation init`
**join**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation join ENDPOINT`
**leave**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation leave`
**peers**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation peers`
**send**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation send NODE_ID MSG_TYPE PAYLOAD [--max-hops N] [--max-tokens N] [--max-usd N] [--hop-count N] [--spent-tokens N] [--spent-usd N]`
**status**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation status`
**audit**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation audit --compliance MODE --since DATE`
**trust**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation trust NODE_ID --review`
**config**: `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation config --pii-policy PATH`
