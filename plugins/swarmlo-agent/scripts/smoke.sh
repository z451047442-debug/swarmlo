...
step "1. plugin.json is swarmlo-agent 0.2.1 with both-runtime keywords"
P="$ROOT/.claude-plugin/plugin.json"
v=$(grep -E '"version"' "$P" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
n=$(grep -E '"name"' "$P" | grep -oE 'swarmlo-agent' | head -1)
if [[ "$v" != "0.2.1" ]]; then bad "expected version 0.2.1, got '$v'";
elif [[ "$n" != "swarmlo-agent" ]]; then bad "expected name swarmlo-agent";
else
  miss=""
...
step "5. README defers to swarmlo-agentdb namespace convention"
