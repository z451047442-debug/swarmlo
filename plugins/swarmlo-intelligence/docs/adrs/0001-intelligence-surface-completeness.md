...
- **`hooks_intelligence_attention`** — attention-mechanism dispatch (the same primitive `swarmlo-ruvector` surfaces via `attention list`). Plugin makes no mention.
...
`swarmlo-agentdb` ADR-0001 (just landed) introduced a namespace convention with three reserved namespaces: `pattern` (ReasoningBank fallback), `claude-memories` (Claude Code bridge), `default`. **The intelligence pipeline writes to `pattern`** (via `agentdb_pattern-store` and the `hooks post-task --train-neural` path). The plugin should explicitly cite the convention so consumers don't reinvent it.

There's also a pluralization gotcha worth surfacing here: ruvector's neural/pretrain hooks write to `patterns` (plural), distinct from ReasoningBank's `pattern`. `swarmlo-agentdb` documents this; `swarmlo-intelligence` should reference the documentation.

### Why now

`swarmlo-intelligence` is the **user-facing** surface for the self-learning system that the rest of the plugin family relies on. Documentation drift here means:
...
4. Downstream plugins (`swarmlo-browser` ADR-0001 references trajectory recording; `swarmlo-agentdb` references `pattern` namespace writes) lose their anchor doc.

We just fixed analogous drift in `swarmlo-ruvector` (ADR-0001) and `swarmlo-agentdb` (ADR-0001). Same pattern applies here.
...
`plugin.json` moves `0.1.0 → 0.3.0` (matches the swarmlo-agentdb cadence and reflects that this is a **substantial scope expansion**, not a patch). Keywords gain `microlora`, `ewc`, `attention`, `moe`, `pattern-transfer`, `model-routing`, `mcp`. Description rewrites to explicitly enumerate the three families and the IPFS transfer.
...
- **Hook integration** — table mapping `post-task --train-neural`, `pretrain`, `pre-task` to which intelligence tools fire. References swarmlo-agentdb's reserved namespaces (`pattern` for ReasoningBank, `patterns` for neural-train).
- **Namespace coordination** — explicit deferral to `swarmlo-agentdb` ADR-0001 §"Namespace convention"; reproduces the pluralization gotcha (`pattern` vs `patterns`).
...
- **Compatibility** — pinned to `@claude-flow/cli` v3.6 (matches swarmlo-agentdb).
...
10. Pluralization gotcha referenced (`pattern` vs `patterns`) — defers to swarmlo-agentdb.
...
- `grep -q "4-step intelligence pipeline" plugins/swarmlo-intelligence/README.md`
- `grep -q "hooks_transfer" plugins/swarmlo-intelligence/skills/intelligence-transfer/SKILL.md`
- `grep -qE "RETRIEVE.+JUDGE.+DISTILL.+CONSOLIDATE" plugins/swarmlo-intelligence/README.md`
...
- Namespace coordination with `swarmlo-agentdb` is explicit; downstream plugins inherit one consistent story.
...
```bash
bash plugins/swarmlo-intelligence/scripts/smoke.sh
# Expected: "16 passed, 0 failed" (13 contract checks + 3 doc invariants)
```

## Related

- `plugins/swarmlo-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent
- `plugins/swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + reserved namespaces (`pattern`/`claude-memories`/`default`); pluralization gotcha; hook integration table
- `plugins/swarmlo-browser/docs/adrs/0001-browser-skills-architecture.md` — uses ruvector trajectory hooks (a primary learning input for this plugin)
