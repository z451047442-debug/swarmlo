...
Usage: /swarmlo-browser <verb> [args]
...
1. **ls [--query "<text>"] [--host <host>] [--verdict pass|fail|partial]**
   ...
   ```bash
   npx -y swarmlo-cli@3.39.1 memory list --namespace browser-sessions
   # or with semantic filter:
   npx -y swarmlo-cli@3.39.1 memory search --namespace browser-sessions --query "QUERY"
   ```
...
4. **export `<session-id>` [-o <path>] [--federate]**
   ...
   ```
   With `--federate`, also push via the `swarmlo-federation` plugin.
...
7. **doctor**
   ...
   - Playwright runner reachable (`mcp__plugin_swarmlo-core_swarmlo__browser_open` against `about:blank`)
   - AgentDB controllers initialized (`mcp__plugin_swarmlo-core_swarmlo__agentdb_health`)
   - AIDefence loaded (`mcp__plugin_swarmlo-core_swarmlo__aidefence_stats`)
