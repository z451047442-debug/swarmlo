...
//   - ensureCachedInstall()   one-time `npm install --prefix ~/.swarmlo/<name>-cache-<pin>`
...
//   - SWARMLO_METAHARNESS_CACHE_BASE   overrides ~/.swarmlo as the cache root
//   - SWARMLO_METAHARNESS_SKIP_LOCAL=1 disables local node_modules resolution
...
// npx shim means "upstream unavailable", never a swarmlo bug.
...
/** Cache root — ~/.swarmlo unless the test seam overrides it. */
export function cacheBaseDir() {
  return process.env.SWARMLO_METAHARNESS_CACHE_BASE || join(homedir(), '.swarmlo');
}
 ...
 * user's swarmlo install shipped with). Returns the package dir or null.
...
export function findLocalPackageDir(pkg, pinVersion) {
  if (process.env.SWARMLO_METAHARNESS_SKIP_LOCAL === '1') return null;
 ...
 * rule #3: swarmlo continues to function when MetaHarness is absent.
 */
