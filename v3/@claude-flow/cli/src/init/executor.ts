  ...
  try {
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use Swarmlo'
      };
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use Swarmlo'
      };
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add swarmlo allow rules)
 ...
 * that already declares a `swarmlo`-keyed MCP server. We use this to skip
 ...
 * issue describes.
 *
 * Returns the path of the file that already declares `swarmlo`/`claude-flow` (so we can
 * surface it in the skipped-message), or null if none found.
 */
function detectExistingSwarmloMCP(targetDir: string): string | null {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // and the current generator would write `swarmlo` (#2612).
      ...
      //     existing `claude-flow` or `swarmlo` registration in this repo is
...
async function writeMCPConfig(
  ...
  // see #2985); a stray `swarmlo`-keyed entry pointing at the same
  ...
  if (!options.force) {
    const existingSwarmloPath = detectExistingSwarmloMCP(targetDir);
    if (existingSwarmloPath) {
      result.skipped.push(`.mcp.json (existing swarmlo/claude-flow MCP registration found at ${existingSwarmloPath} — would create duplicate; pass --force to write anyway)`);
      return;
    }
  }
...
async function writeHelpers(
  ...
  if (sourceHelpersDir && fs.existsSync(sourceHelpersDir)) {
    ...
    // #2132: Always generate swarmlo-hook.cjs regardless of source copy path.
    // The source helpers dir may not contain this file (it lives in
    // plugins/swarmlo-core/scripts/, not .claude/helpers/), but it must
    // always be present so Windows users can use the node-based shim.
    const swarmloHookDest = path.join(helpersDir, 'swarmlo-hook.cjs');
    if (!fs.existsSync(swarmloHookDest) || options.force) {
      fs.writeFileSync(swarmloHookDest, generateSwarmloHookCjs(), 'utf-8');
      result.created.files.push('.claude/helpers/swarmlo-hook.cjs');
    } else {
      result.skipped.push('.claude/helpers/swarmlo-hook.cjs');
    }
...
async function writeRuntimeConfig(
  ...
  const config = `# Swarmlo V3 Runtime Configuration
...
async function writeInitialMetrics(
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use Swarmlo. Run: npx swarmlo@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
...
/**
 * Write CAPABILITIES.md - comprehensive overview of all Swarmlo features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilities = `# Swarmlo V3 - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
> Full documentation: https://github.com/z451047442-debug/swarmlo
...
\`\`\`bash
# Add Swarmlo MCP
claude mcp add swarmlo -- npx -y swarmlo@latest mcp start
...
# Swarm
npx swarmlo@latest swarm init --topology hierarchical --max-agents 8
npx swarmlo@latest swarm status

# Agents
npx swarmlo@latest agent spawn -t coder
npx swarmlo@latest agent list

# Memory
npx swarmlo@latest memory search --query "patterns"

# Hooks
npx swarmlo@latest hooks pre-task --description "task"
npx swarmlo@latest hooks worker dispatch --trigger optimize
\`\`\`
...
async function writeClaudeMd(
  ...
  if (fs.existsSync(claudeMdPath) && !options.force) {
    ...
    if (fs.existsSync(claudeMdPath)) {
      const backupBase = `${claudeMdPath}.pre-swarmlo`;
      ...
      console.warn(`[swarmlo init] Existing CLAUDE.md backed up to ${path.basename(backupPath)} before overwrite`);
    }
  ...
  // Also write/append global ~/.claude/CLAUDE.md so swarmlo tools are used automatically (#1497).
  ...
  if (homeDir && !options.skipGlobalClaudeMd) {
    ...
    const swarmloBlock = [
      '',
      '# Swarmlo Integration (auto-generated by swarmlo init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke swarmlo MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('Swarmlo Integration')) {
          fs.appendFileSync(globalClaudeMd, swarmloBlock);
          result.created.files.push('~/.claude/CLAUDE.md (appended swarmlo block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, swarmloBlock.trimStart(), 'utf-8');
