...
const searchCommand: Command = {
  ...
  options: [
    ...
    {
      name: 'threshold',
      // ADR-382: no default here — when the user doesn't pass --threshold,
      // the command derives a model-aware default (bge-m3: 0.15, others 0.7).
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // ADR-382: model-aware default threshold — bge-m3's dense space is
    // looser than MiniLM's (related pairs measured 0.17-0.22), so the
    // legacy 0.7 default would crush every hit. Explicit --threshold wins.
    const { resolveEmbeddingModel } = await import('../memory/embedding-models.js');
    const { resolveConfiguredEmbedding } = await import('../memory/embedding-guard.js');
    const modelDefaultThreshold = resolveEmbeddingModel(resolveConfiguredEmbedding().model).defaultThreshold;
    const threshold = ctx.flags.threshold as number ?? modelDefaultThreshold ?? 0.7;
