#!/usr/bin/env node
/**
 * Smoke test for z451047442-debug/swarmlo#2132 — Windows-compatible init hook generation.
 *
 * Verifies that `swarmlo init` generates a .claude/settings.json containing
...
// Run swarmlo init in a temporary directory
const tmpDir = mkdtempSync(join(tmpdir(), 'swarmlo-smoke-win-init-'));
console.log(`Running swarmlo init in: ${tmpDir}`);
...
if (!existsSync(settingsPath)) {
  ...
  console.error('FAIL: settings.json not generated by swarmlo init');
  process.exit(1);
}
...
if (IS_WINDOWS) {
  ...
  // Check that swarmlo-hook.cjs was deployed (new in #2132)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'swarmlo-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/swarmlo-hook.cjs exists (#2132)');
  ...
  // swarmlo-hook.cjs is always deployed now (cross-platform shim always available)
  const shimPath = join(tmpDir, '.claude', 'helpers', 'swarmlo-hook.cjs');
  assert(existsSync(shimPath), '.claude/helpers/swarmlo-hook.cjs exists (#2132)');
}
