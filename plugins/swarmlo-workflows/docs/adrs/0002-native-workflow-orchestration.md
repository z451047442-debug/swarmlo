...
[ADR-0001](./0001-workflows-contract.md) established `swarmlo-workflows` as the canonical wrapper for the **10 `workflow_*` MCP tools** (`v3/@claude-flow/cli/src/mcp-tools/workflow-tools.ts`). That surface is *declarative and persisted*: a workflow definition is created, then run/paused/resumed/cancelled through a state machine, with state indexed in the `workflows-state` AgentDB namespace.
...
- `plugins/swarmlo-loop-workers/docs/adrs/0001-loop-workers-contract.md` — sibling automation surface (recurring loops)
- `plugins/swarmlo-sparc/docs/adrs/0001-sparc-contract.md` — SPARC phase transitions as workflows
