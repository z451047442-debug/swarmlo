 ...
 *
 * `swarmlo-cost-tracker` records token usage per session in a causation graph:
...
export function costAttributionGraphId(sessionId?: string): string {
  return sessionId ? `swarmlo-cost-tracker:causation:${sessionId}` : 'swarmlo-cost-tracker:causation:global';
}

export class CostAttributionAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'swarmlo-cost-tracker';
