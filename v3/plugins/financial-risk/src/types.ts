...
export const MarketRegimeInputSchema = z.object({
  marketData: z.object({
    prices: z.array(z.number().finite()).max(10000),
  ...
  // Short lookback windows only need a few price points; longer horizons need more.
  const minPrices = data.lookbackPeriod <= 30 ? 3 : 6;
  if (data.marketData.prices.length < minPrices) {
    ctx.addIssue({
      code: z.ZodIssueCode.too_small,
      minimum: minPrices,
      type: 'array',
      inclusive: true,
      path: ['marketData', 'prices'],
      message: `At least ${minPrices} price points required for lookback period ${data.lookbackPeriod}`,
    });
  }
});
