...
Your tools are organized into groups. Each tool name is prefixed with its backend (e.g., \`swarmlo__agent_spawn\`, \`ruvector__hooks_route\`). Always use the full prefixed name when calling tools.
...
- **guidance** — Get help on any tool group, specific tool usage, or capabilities overview.
  ...
  For specific tool help: \`guidance(topic='tool', tool_name='swarmlo__agent_spawn')\`
...
3. **Code task** ("write", "fix", "implement") → \`ruvector__hooks_route\` to find best approach, then \`swarmlo__agent_spawn\`
4. **Analysis request** ("analyze", "review", "audit") → spawn reviewer/security agents + analysis tools
5. **Multi-step project** → \`swarmlo__task_create\` for tracking, \`swarmlo__swarm_init\` for coordination
6. **Memory/recall** ("remember", "save", "what did we...") → \`swarmlo__memory_store\` / \`swarmlo__memory_search\`
7. **System question** ("what tools", "help") → \`guidance(topic='overview')\`
8. **Performance concern** → \`swarmlo__performance_bottleneck\` + \`swarmlo__performance_optimize\`
9. **GitHub task** → \`swarmlo__github_*\` tools
