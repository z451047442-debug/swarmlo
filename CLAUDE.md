# Claude Code Configuration - Swarmlo V3

> Public release train: `@claude-flow/cli`, `claude-flow`, and `swarmlo`.
...
Swarmlo is the coordination ledger and policy decision point. Claude Code
executes code, tests, commands, and file changes. A Swarmlo coordination call
...
- The normal public release train is exactly THREE packages:
  `@claude-flow/cli`, `claude-flow`, and `swarmlo`.
...
- Publish order: `@claude-flow/cli` first, then `claude-flow` (umbrella), then `swarmlo` (alias umbrella)
...
granular access token ("swarmlo publishjing", expires 2026-10-28) with
`package: write` + `bypass_2fa: true`, scoped broadly enough to cover
`@claude-flow/cli`, `claude-flow`, and `swarmlo` (plus the `cognitum`/
...
secret name `swarmlo-helpers-signing-key`:

```bash
cd v3/@claude-flow/cli
SWARMLO_HELPERS_SIGNING_SECRET=swarmlo-helpers-signing-key SWARMLO_HELPERS_SIGNING_PROJECT=ruv-dev \
  npm publish
```

(`ruv-dev` also holds `swarmlo-config-signing-key`; do not replace the existing
...
- NEVER invoke `gcloud secrets versions access` in a way that lets the payload reach
  tool output. Use the built-in `SWARMLO_HELPERS_SIGNING_SECRET` path above, or pipe
  directly into the signer:
  `gcloud secrets versions access latest --secret=swarmlo-helpers-signing-key --project=ruv-dev | node scripts/sign-helpers.mjs --stdin-key`.
- `--stdin-key` refuses interactive entry, validates Ed25519 key type, and never
  echoes parser input. A local file via `SWARMLO_HELPERS_SIGNING_KEY` remains the
  air-gapped fallback.
- If a rotation IS needed, keep the private half in `~/.swarmlo/helpers-signing.key`
...
Option 1 is what worked for v3.29.0. Track proper fix in z451047442-debug/swarmlo issue for
...
cd /Users/cohen/Projects/swarmlo                    # or your repo root
npm version 3.7.1 --no-git-tag-version
npm publish
npm dist-tag add claude-flow@3.7.1 alpha
npm dist-tag add claude-flow@3.7.1 v3alpha

# STEP 3: Publish swarmlo wrapper (CRITICAL — DON'T FORGET — this is what users run)
cd swarmlo
npm version 3.7.1 --no-git-tag-version
npm publish
npm dist-tag add swarmlo@3.7.1 alpha
npm dist-tag add swarmlo@3.7.1 v3alpha
```

**Verification (run before telling user publishing is complete):**

```bash
for pkg in @claude-flow/cli claude-flow swarmlo; do
...
| `swarmlo` | `latest` | `npx swarmlo@latest` |
| `swarmlo` | `alpha` | `npx swarmlo@alpha` (legacy compat) |
| `swarmlo` | `v3alpha` | `npx swarmlo@v3alpha` (legacy compat) |

- Never forget the `swarmlo` package — it's the thin wrapper users actually run via `npx swarmlo`
- The legacy `alpha` and `v3alpha` tags MUST stay pointed at the latest stable so old install commands keep working
- `swarmlo` source is in `/swarmlo/` — it depends on `@claude-flow/cli`
- Also remember to update `swarmlo/package.json` overrides when adding new pinned transitives (see #2112 lesson — root overrides do NOT propagate to the published `swarmlo` wrapper)
...
Swarmlo integrates with the upstream `metaharness` / `@metaharness/*` ecosystem as a sibling agent-harness scaffolding system (same author, designed around swarmlo's primitives). MetaHarness packages are optional peer dependencies and are never required at runtime.

### Architectural constraint (load-bearing)

**Swarmlo remains operational if every MetaHarness package is removed.** Four rules:
...
```bash
# CLI subcommands (npx swarmlo metaharness …)
npx swarmlo metaharness score                      # 5-dim readiness scorecard
npx swarmlo metaharness genome                     # 7-section categorical report
npx swarmlo metaharness mcp-scan --fail-on high    # static security findings
npx swarmlo metaharness threat-model               # enterprise threat report
npx swarmlo metaharness oia-audit --alert-on-worst high
                                                 # composite weekly audit → memory
npx swarmlo metaharness audit-list --since 30d     # enumerate audit records
npx swarmlo metaharness audit-trend \              # diff two audits (drift)
  --baseline-key <a> --current-key <b> --alert-on-worsening \
  --alert-on-distance-below 0.85               # iter 38 — structural-distance gate (ADR-152 §3.1)
npx swarmlo metaharness similarity \               # iter 36 — ADR-152 §3.1 weighted similarity
  --a a.json --b b.json [--per-dimension] [--alert-below 0.5]
npx swarmlo metaharness drift-from-history \       # iter 53 — 1-command drift (composes 3 primitives)
...
npx swarmlo metaharness redblue init               # @metaharness/redblue — scaffold redblue.yaml
npx swarmlo metaharness redblue run --mock-judge --tests 10
                                                 # $0 marker-fixture path (CI / offline)
npx swarmlo metaharness redblue run --tests 50 --patch
                                                 # real model judge (needs OPENROUTER_API_KEY,
                                                 #   capped by max_cost_usd, default $3)
npx swarmlo metaharness redblue attack prompt --count 3
                                                 # preview generated attack cases (no target call)
npx swarmlo metaharness redblue patch --mock-judge # baseline → blue-team patch → retest delta
npx swarmlo metaharness redblue report --in report.json
                                                 # render existing report as markdown
npx swarmlo metaharness learn --host claude-code --model haiku --slice slices/lite.json
...
npx swarmlo metaharness gepa --op genome           # darwin@0.8.0 GEPA library — load + validate
                                                 #   the shipped cand-6 genome (or --path <f>)
npx swarmlo metaharness gepa --op render           # genome → the system prompt it compiles to
npx swarmlo metaharness gepa --op analyze --transcript run.json
                                                 # classify failure modes in a transcript
npx swarmlo metaharness evolve --bench .harness/bench.json
                                                 # Darwin proposes candidates; governed gates decide
npx swarmlo metaharness bench verify --path .harness/bench.json
                                                 # create or verify stable benchmark corpora
npx swarmlo metaharness flywheel run --proposer auto --max-concurrency 2
                                                 # bounded concurrent evaluation; does not promote
npx swarmlo metaharness flywheel receipts          # inspect immutable evaluation receipts
npx swarmlo metaharness flywheel promote <receipt-id> \
  --public-key ./approved-ed25519-public.pem --confirm
                                                 # explicit policy-authorized atomic promotion

# Dedicated command
npx swarmlo eject --name my-harness                # lift swarmlo project → standalone harness
                                                 # dry-run by default; refuses in-repo target

# Doctor health check
npx swarmlo doctor --component metaharness         # report metaharness availability + version
...
- `metaharness-ci.yml` — score / mcp-scan / router-compat / eject-dryrun jobs on every PR touching `plugins/swarmlo-metaharness/**`
...
- [Issue #2399](https://github.com/z451047442-debug/swarmlo/issues/2399) — phase tracker
