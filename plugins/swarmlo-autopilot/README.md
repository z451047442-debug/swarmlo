...
Combines Swarmlo's 10 autopilot MCP tools with Claude Code's native `/loop` + `ScheduleWakeup` for persistent, cache-aware task completion loops.

## Install

```
/plugin marketplace add github.com/z451047442-debug/swarmlo
/plugin install swarmlo-autopilot@swarmlo
```
...
- **Verification:** `bash plugins/swarmlo-autopilot/scripts/smoke.sh` is the contract.
...
This plugin owns the `autopilot-patterns` AgentDB namespace (kebab-case, follows the convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.

`autopilot_learn` writes to this namespace via `agentdb_pattern-store` semantics — see [swarmlo-intelligence ADR-0001](../swarmlo-intelligence/docs/adrs/0001-intelligence-surface-completeness.md) for the 4-step pipeline this feeds (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE).
...
- [`ADR-0001` — swarmlo-autopilot plugin contract](./docs/adrs/0001-autopilot-contract.md)
