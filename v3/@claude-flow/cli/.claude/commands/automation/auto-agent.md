...
```bash
npx swarmlo-cli@latest auto agent [options]
```
...
```bash
npx swarmlo-cli@latest auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx swarmlo-cli@latest auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx swarmlo-cli@latest auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx swarmlo-cli@latest auto agent -t "Fix bug in login" -s minimal
```
