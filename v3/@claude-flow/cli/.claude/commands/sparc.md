...
```bash
# Run SPARC orchestrator (default)
npx swarmlo-cli@latest sparc "build complete authentication system"

# Run a specific mode
npx swarmlo-cli@latest sparc run architect "design API structure"
npx swarmlo-cli@latest sparc run tdd "implement user service"

# Execute full TDD workflow
npx swarmlo-cli@latest sparc tdd "implement user authentication"

# List all modes with details
npx swarmlo-cli@latest sparc modes --verbose
...
```bash
# Store specifications
npx swarmlo-cli@latest memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec
