...
4. Store normalized data via `mcp__plugin_swarmlo-core_swarmlo__memory_store --namespace market-data` (the `memory_*` family is namespace-routed; `agentdb_hierarchical-*` routes by tier and ignores namespace strings — see skills/market-ingest/SKILL.md)
5. Add vectors to HNSW index via `mcp__plugin_swarmlo-core_swarmlo__ruvllm_hnsw_add`
...
4. Store detected patterns via the dual-path pattern (see skills/market-pattern/SKILL.md): `mcp__plugin_swarmlo-core_swarmlo__agentdb_pattern-store` (typed, ReasoningBank-routed — do NOT pass `namespace`) AND `mcp__plugin_swarmlo-core_swarmlo__memory_store --namespace market-patterns` (namespace-routed; `agentdb_pattern-*` ignores namespace strings)
...
1. Search HNSW index via `mcp__plugin_swarmlo-core_swarmlo__ruvllm_hnsw_route` for vectors matching the pattern
