#!/usr/bin/env bash
# swarmlo-hook.sh — resilient invoker for swarmlo CLI hook subcommands (#1921).
#
...
#   1. prefers an already-installed `swarmlo` / `claude-flow` binary (no npx,
...
# Usage: swarmlo-hook.sh <hook-subcommand> [args…]   (the literal `hooks`
...
if command -v swarmlo >/dev/null 2>&1; then
  run swarmlo hooks "$@"
