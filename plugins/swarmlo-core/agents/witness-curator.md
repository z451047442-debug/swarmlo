...
`sha256(gitCommit + ':swarmlo-witness/v1')` — no committed private key.
...
Toolkit lives in `plugins/swarmlo-core/scripts/witness/`:
...
3. Run `node plugins/swarmlo-core/scripts/witness/regen.mjs --dry-run` first
