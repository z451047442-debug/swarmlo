...
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation init` -- generate keypair, create config
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation join <endpoint>` -- connect to peer
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation peers` -- list peers with trust levels
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation status` -- health dashboard
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation audit --compliance hipaa` -- audit logs
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation trust <node-id> --review` -- trust breakdown
- `npx -y -p @claude-flow/plugin-agent-federation@latest swarmlo-federation send <node-id> <msg-type> <payload> [--max-hops N] [--max-tokens N] [--max-usd N]` -- delegate with budget guardrails
...
```bash
npx swarmlo-cli@3.39.1 memory store --namespace federation --key "peer-NODEID" --value "TRUST_HISTORY"
```
