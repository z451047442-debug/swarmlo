 ...
 *
 * Wraps the Wedge-8 path from ADR-123 (`mcp__swarmlo-sublinear__solve`,
 ...
 *   - At ADR-126 Phase 3 write-time, the `swarmlo-sublinear` plugin
 *     (ADR-123 Phase 1) has not yet been published on the IPFS registry.
 *     The MCP tool `mcp__swarmlo-sublinear__solve` may not be invocable
 ...
 *   `mcp__swarmlo-sublinear__solve` dispatch surface is reachable from this
 *   runtime. Two probes are tried, in order:
 *     1) globalThis['mcp__swarmlo-sublinear__solve'] is a function — this
 *        is how the swarmlo MCP harness mounts tools into the agent runtime.
 *     2) process.env.SWARMLO_SUBLINEAR_NATIVE === '1' — manual override for
...
export interface SolveResult {
   ...
   *  - `cg-sublinear-native` — native dispatch via mcp__swarmlo-sublinear__solve
   *  - `cg-local`            — embedded JS CG fallback
   */
...
export class SublinearAdapter {
  /**
   * Detection — is `mcp__swarmlo-sublinear__solve` reachable in this runtime?
   *
   * Two probes, in priority order:
   *   1) globalThis['mcp__swarmlo-sublinear__solve'] is a function. This is the
   *      convention the swarmlo MCP harness uses to mount native tools into the
  ...
  static detectSublinearTool(): boolean {
    try {
      ...
      const tool = g['mcp__swarmlo-sublinear__solve'];
    ...
    try {
      ...
      const envFlag = typeof process !== 'undefined' && process.env
        ? process.env.SWARMLO_SUBLINEAR_NATIVE
        : undefined;
  ...
  async solveCG(
    ...
    if (SublinearAdapter.detectSublinearTool()) {
      try {
        ...
        // have set SWARMLO_SUBLINEAR_NATIVE=1 in an environment where the tool
...
async function callMcpSolve(
  ...
  const g = globalThis as unknown as Record<string, unknown>;
  const tool = g['mcp__swarmlo-sublinear__solve'] as
    | ((args: unknown) => Promise<unknown>)
    | undefined;
  if (typeof tool !== 'function') {
    throw new Error('mcp__swarmlo-sublinear__solve not available');
  }
  ...
  if (!out || !Array.isArray(out.solution)) {
    throw new Error('mcp__swarmlo-sublinear__solve returned invalid shape');
  }
  return {
