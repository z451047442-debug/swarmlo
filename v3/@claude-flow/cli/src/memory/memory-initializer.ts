...
import { getBgeEmbedder, getBgeStatus, applyHfEndpoint } from './bge-embedder.js';
import { resolveEmbeddingModel, isBgeFamily, DEFAULT_EMBEDDING_MODEL, BRIDGE_EMBEDDING_MODEL } from './embedding-models.js';
import {
  assertDimensionCompatible,
  dimensionMismatchError,
  readStoredVectorInfoFromHandle,
  resolveConfiguredEmbedding,
  sanitizeModelName,
} from './embedding-guard.js';
...
// ============================================================================
// HNSW INDEX SINGLETON (measured ~1.9x-4.7x vs brute-force vector search)
...
export async function addToHNSWIndex(
  ...
  // ADR-053: Try AgentDB v3 bridge first — only when the bridge is the
  // active memory backend (ADR-382: non-explicit MiniLM config).
  const configuredBridge = resolveConfiguredEmbedding();
  const bridge =
    !configuredBridge.explicit && configuredBridge.model === BRIDGE_EMBEDDING_MODEL
      ? await getBridge()
      : null;
  if (bridge) {
...
/**
 * Search HNSW index (measured ~1.9x-4.7x vs brute-force)
...
export function getInitialMetadata(
  ...
  embedding: { model: string; dimensions: number } = {
    ...
    dimensions: resolveEmbeddingModel(DEFAULT_EMBEDDING_MODEL).dim,
  },
): string {
  const safeModel = sanitizeModelName(embedding.model);
  return `
...
-- Create default vector index configuration. Dimension matches the
-- configured embedding model (default Xenova/all-MiniLM-L6-v2, 384-dim);
-- HNSW rejects inserts whose dim does not match this row, so a wrong value
-- here breaks every memory_store --vector and memory_search (#1947).
-- ADR-382: seeded from the configured model so a bge-m3 install gets
-- 1024-dim rows and the store-side guard stays consistent.
INSERT OR IGNORE INTO vector_indexes (id, name, dimensions) VALUES
  ('default', 'default', ${embedding.dimensions}),
  ('patterns', 'patterns', ${embedding.dimensions});
`;
}
...
async function activateControllerRegistry(
  ...
  try {
    // ADR-382: skip bridge controllers when the bridge is not the active
    // memory backend (bge-m3 default / any explicit config).
    const configuredBridge = resolveConfiguredEmbedding();
    const bridge =
      !configuredBridge.explicit && configuredBridge.model === BRIDGE_EMBEDDING_MODEL
        ? await getBridge()
...
export async function initializeMemoryDatabase(options: {
  ...
  try {
    ...
    if (usedSqlJs && db) {
      ...
      db.run(getInitialMetadata(backend, resolveConfiguredEmbedding()));
      ...
      fs.writeFileSync(schemaPath, MEMORY_SCHEMA_V3 + '\n' + getInitialMetadata(backend, resolveConfiguredEmbedding()));
...
/** ADR-382: cache key of the loaded state (model:dim) — a model switch reloads. */
...
export async function loadEmbeddingModel(options?: {
  modelPath?: string;
  verbose?: boolean;
  /** ADR-382: explicit model override (e.g. 'Xenova/bge-m3'). */
  modelName?: string;
  /** ADR-382: expected dim; branches whose dims differ fail fast. */
  dimension?: number;
}): Promise<{
  success: boolean;
  dimensions: number;
  ...
  const { verbose = false, modelName: optModelName, dimension: optDim } = options || {};
  const startTime = Date.now();

  // ADR-382: model from options > env > embeddings.json > default.
  const configured = resolveConfiguredEmbedding();
  const resolvedModelName = optModelName ?? configured.model;
  const envDim = process.env.CLAUDE_FLOW_EMBEDDING_DIMENSION
    ? Number(process.env.CLAUDE_FLOW_EMBEDDING_DIMENSION)
    : undefined;
  const explicitDim = optDim ?? envDim;
  const validExplicitDim =
    Number.isFinite(explicitDim as number) && (explicitDim as number) > 0
      ? (explicitDim as number)
      : undefined;
  const modelKey = `${resolvedModelName}:${validExplicitDim ?? 'any'}`;

  // Shared exit for every successful branch: stamps the cache key and
  // enforces the explicit dimension guard (ADR-382 layer 1).
  const finishBranch = (
    dims: number,
    name: string,
  ): { success: boolean; dimensions: number; modelName: string; loadTime?: number; error?: string } => {
    loadedModelKey = modelKey;
    if (validExplicitDim !== undefined && dims !== validExplicitDim) {
      return {
        success: false,
        dimensions: dims,
        modelName: name,
        loadTime: Date.now() - startTime,
        error: dimensionMismatchError(resolvedModelName, validExplicitDim, dims),
      };
    }
    return {
      success: true,
  ...
  // Already loaded — only reuse when the requested model/dim key matches.
  if (embeddingModelState?.loaded && loadedModelKey === modelKey) {
    return {
      success: true,
  ...
  // ADR-053: Try AgentDB v3 bridge first — but ONLY when the resolved model
  // is the bridge's own MiniLM and nothing was explicitly configured.
  // ADR-382: the bridge hardcodes MiniLM (384-dim), so with the bge-m3
  // default (or any explicit override) the local chain must serve instead.
  if (!configured.explicit && configured.model === BRIDGE_EMBEDDING_MODEL) {
    const bridge = await getBridge();
    if (bridge) {
      const bridgeResult = await bridge.bridgeLoadEmbeddingModel();
      if (bridgeResult && bridgeResult.success) {
        if (validExplicitDim !== undefined && bridgeResult.dimensions !== validExplicitDim) {
          return {
            success: false,
            dimensions: 0,
            modelName: 'none',
            error: dimensionMismatchError(resolvedModelName, validExplicitDim, bridgeResult.dimensions),
          };
        }
        loadedModelKey = modelKey;
  ...
  if (isBgeFamily(resolvedSpec)) {
    ...
    if (bge) {
      ...
      embeddingModelState = {
        loaded: true,
        // Wrap to { data } — generateLocalEmbedding checks output?.data and
        // falls through to the hash embedder for bare arrays (a Float32Array
        // is NOT Array.isArray).
        model: (text: string) => bge.embed(text).then((v) => ({ data: v })),
        tokenizer: null,
        dimensions: dim
      };
      return finishBranch(dim, resolvedModelName);
    }
    // BGE configured but unavailable — fail loudly instead of serving
    // wrong-pooling vectors from the generic pipeline chain.
    return {
      success: false,
      dimensions: 0,
      modelName: resolvedModelName,
      error: `BGE embedder failed to load: ${getBgeStatus(resolvedModelName).error}`,
    };
  }
  ...
  try {
...

    {
      const tryLoad = async (specifier: string): Promise<Record<string, unknown> | null> => {
        try {
          ...
          applyHfEndpoint(mod); // ADR-382: HF mirror endpoint override
          return mod;
        }
        catch {
          // 2026-08-17: HF v4 ESM entry can fail to load on some Node builds
          // (onnxruntime-common CJS interop) — fall back to its CJS entry.
          if (specifier === '@huggingface/transformers') {
            try {
              const mod = createRequire(import.meta.url)(specifier) as Record<string, unknown>;
              applyHfEndpoint(mod);
              return mod;
            } catch {
              return null;
    ...
    if (pipelineFn && transformersSource) {
      ...
      try {
        if (verbose) {
          console.log(`Loading ONNX embedding model via ${transformersSource} (${resolvedModelName})...`);
        }
        const embedder = await pipelineFn('feature-extraction', resolvedModelName);

        const dims = resolveEmbeddingModel(resolvedModelName).dim;
        embeddingModelState = {
          loaded: true,
          model: embedder,
          tokenizer: null,
        ...
        return finishBranch(dims, resolvedModelName);
      } catch (err) {
        if (verbose) {
          console.warn(
    ...
    if (reasoningBank?.computeEmbedding) {
      ...
      return finishBranch(768, 'agentic-flow/reasoningbank');
    }
      ...
      try {
        ...
        if (onnxEmb?.embed) {
          ...
          if (probe && probe.length > 0 && (Array.isArray(probe) ? probe.some((v: number) => v !== 0) : true)) {
            ...
            return finishBranch(probe.length || 384, 'ruvector/onnx');
          }
        }
      } catch {
    ...
    if (agenticFlow && (agenticFlow as any).embeddings) {
      ...
      return finishBranch(768, 'agentic-flow');
    }
    ...
    return finishBranch(128, 'hash-fallback');
  } catch (error) {
    return {
      success: false,
      dimensions: 0,
...
export async function generateEmbedding(text: string): Promise<{
  ...
  // ADR-053: Try AgentDB v3 bridge first — default path only (ADR-382: an
  // explicit model override must go local; the bridge hardcodes MiniLM).
  const configuredBridge = resolveConfiguredEmbedding();
  const bridge =
    !configuredBridge.explicit && configuredBridge.model === BRIDGE_EMBEDDING_MODEL
      ? await getBridge()
...
export async function storeEntry(options: {
  ...
  try {
    ...
    await withMemoryDbLock(dbPath, async () => {
      ...
      // ADR-382 layer 2: refuse writes whose embedding dims don't match the
      // stored vector index — mixing vector spaces silently breaks search.
      if (embeddingDimensions != null) {
        const stored = readStoredVectorInfoFromHandle(db, namespace);
        const compat = assertDimensionCompatible(
          stored,
          embeddingDimensions,
 ...
 * Uses HNSW index (measured ~1.9x-4.7x vs brute force) when available
 */
export async function searchEntries(options: {
  ...
  // ADR-382: model-aware default threshold — bge-m3's dense space needs a
  // lower cutoff than the MiniLM-tuned 0.3 (registry defaultThreshold).
  const specForThreshold = resolveEmbeddingModel(resolveConfiguredEmbedding().model);
  const effectiveThreshold = threshold ?? specForThreshold.defaultThreshold ?? 0.3;
  ...
  try {
    ...
    // Try HNSW search (measured ~1.9x-4.7x vs brute-force)
    ...
    if (hnswResults && hnswResults.length > 0) {
      ...
      let filtered: Array<{ id: string; key: string; content: string; score: number; namespace: string; provenanceType?: string }> =
        hnswResults.filter(r => r.score >= effectiveThreshold);
...
export async function purgeNamespace(options: {
  ...
  return withMemoryDbLock(dbPath, async () => {
    ...
    // read/mutate/rewrite. ADR-382: only when the bridge is the active
    // memory backend (non-explicit MiniLM config).
    const configuredBridge = resolveConfiguredEmbedding();
    const bridge =
      !configuredBridge.explicit && configuredBridge.model === BRIDGE_EMBEDDING_MODEL
        ? await getBridge()
