...
| Swarmlo default (`pre-task` hook) | 4 levels — one-level guard band, configurable in `claude-flow.config.json` |
| Strict-mode env var | `CLAUDE_FLOW_STRICT_NESTING=true` to enforce the swarmlo cap |
...
- **Maps cleanly onto swarmlo's existing orchestrators** — `swarmlo-sparc:sparc-orchestrator` (5 phases ≈ 5 levels), `swarmlo-goals:dossier-investigator` (recursive entity expansion), `v3-queen-coordinator` (hierarchical-mesh top).
...
- **Agent**: `swarmlo-agent:nested-coordinator` — the orchestrator
