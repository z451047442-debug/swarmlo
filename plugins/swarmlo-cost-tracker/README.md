...
```bash
claude --plugin-dir plugins/swarmlo-cost-tracker
```
...
## Federation budget circuit breaker pairing (swarmlo 3.6.25+)
...
- **Verification:** `bash plugins/swarmlo-cost-tracker/scripts/smoke.sh` is the contract.
...
Both follow the kebab-case `<plugin-stem>-<intent>` convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md). Both are accessed via the `memory_*` tool family which routes by namespace.
...
- [`ADR-0001` — swarmlo-cost-tracker plugin contract (namespace-routing fix, federation budget pairing, smoke as contract)](./docs/adrs/0001-cost-tracker-contract.md)
...
- `swarmlo-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `swarmlo-observability` -- Token usage metrics collected via observability instrumentation
- `swarmlo-neural-trader` -- PnL tracking and cost-adjusted return calculation
- `swarmlo-federation` -- Budget circuit breaker on outbound federation_send (ADR-097)
