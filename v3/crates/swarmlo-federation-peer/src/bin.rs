//! `swarmlo-federation-peer` binary entry point.
//!
...
fn main() -> ExitCode {
    if std::env::args().any(|a| a == "--version" || a == "-V") {
        println!("swarmlo-federation-peer {}", env!("CARGO_PKG_VERSION"));
    ...
    {
        eprintln!(
            "swarmlo-federation-peer {} — built without `--features native`.\n\
...
fn native_main() -> ExitCode {
    ...
    use swarmlo_federation_peer::{
        native_gate::AimdsGate,
        native_transport::MidstreamerTransport,
        Dispatcher, FederationMessage, Peer, PeerError,
    };

    let remote = match std::env::var("SWARMLO_FEDERATION_REMOTE_ADDR") {
        Ok(v) => v,
        Err(_) => {
            eprintln!(
                "SWARMLO_FEDERATION_REMOTE_ADDR is required (e.g. hub.local:4433).",
            );
    ...
    // process (the swarmlo MCP server) can ingest verdicts.
        ...
        .build()
    {
        Ok(r) => r,
        Err(e) => {
            eprintln!("swarmlo-federation-peer: failed to build runtime: {e}");
            return ExitCode::from(2);
        }
    };
    ...
    rt.block_on(async move {
        let transport = match MidstreamerTransport::connect(&remote).await {
            Ok(t) => t,
            Err(e) => {
                eprintln!("swarmlo-federation-peer: connect failed: {e}");
        ...
        if let Err(e) = peer.run().await {
            eprintln!("swarmlo-federation-peer: run failed: {e}");
            return ExitCode::from(1);
        }
        ExitCode::SUCCESS
    })
