 ...
 *   - cg_sublinear_native_avg_ms   — native dispatch via mcp__swarmlo-sublinear__solve
...
//
// The native dispatch is reachable iff `mcp__swarmlo-sublinear__solve` is
// mounted on globalThis (or SWARMLO_SUBLINEAR_NATIVE=1 forces an attempt).
...
if (at256.cgNativeAvgMs != null) {
  console.log(`- CG (native) latency at n=256: **${at256.cgNativeAvgMs.toFixed(4)} ms** (via \`mcp__swarmlo-sublinear__solve\`)`);
  ...
  console.log('  - Reasons it can be unreachable: swarmlo daemon not running, swarmlo-sublinear plugin not registered, or the agent sandbox does not mount MCP tools onto globalThis. Set `SWARMLO_SUBLINEAR_NATIVE=1` to force a dispatch attempt.');
}
...
console.log('## Refs');
console.log('');
console.log('- ADR-126 Phase 3 — `plugins/swarmlo-neural-trader/src/sublinear-adapter.ts`');
