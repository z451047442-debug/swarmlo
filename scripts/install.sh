#!/usr/bin/env bash
#
# Swarmlo Installer (formerly Claude Flow)
...
#   curl -fsSL https://cdn.jsdelivr.net/gh/z451047442-debug/swarmlo@main/scripts/install.sh | bash -s -- --global
#   curl -fsSL https://cdn.jsdelivr.net/gh/z451047442-debug/swarmlo@main/scripts/install.sh | bash -s -- --minimal
#
...
VERSION="${SWARMLO_VERSION:-${CLAUDE_FLOW_VERSION:-latest}}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --version=*)
            ...
            ;;
        --help|-h)
            echo "Swarmlo Installer"
            echo ""
            echo "Usage: curl -fsSL .../install.sh | bash -s -- [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --global, -g     Install globally (npm install -g swarmlo)"
...
print_banner() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}  ${BOLD}Swarmlo${NC} — AI Agent Orchestration for Claude Code     ${CYAN}║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
verify_installation() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        VERSION_OUTPUT=$(swarmlo --version 2>/dev/null || claude-flow --version 2>/dev/null || echo "")
        if [ -z "$VERSION_OUTPUT" ]; then
            print_warning "Global command not found in PATH"
            print_substep "Try: ${BOLD}npm install -g swarmlo@${VERSION}${NC}"
...
show_quickstart() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        ...
        echo -e "  ${BOLD}swarmlo init --wizard${NC}"
        echo ""
        echo -e "  ${DIM}# Run system diagnostics${NC}"
        echo -e "  ${BOLD}swarmlo doctor${NC}"
        echo ""
        echo -e "  ${DIM}# Add as MCP server to Claude Code${NC}"
        echo -e "  ${BOLD}claude mcp add swarmlo -- swarmlo mcp start${NC}"
    else
        echo -e "  ${DIM}# Initialize project${NC}"
        echo -e "  ${BOLD}npx swarmlo@latest init --wizard${NC}"
        echo ""
        echo -e "  ${DIM}# Run system diagnostics${NC}"
        echo -e "  ${BOLD}npx swarmlo@latest doctor${NC}"
        echo ""
        echo -e "  ${DIM}# Add as MCP server to Claude Code${NC}"
        echo -e "  ${BOLD}claude mcp add swarmlo -- npx -y swarmlo@latest mcp start${NC}"
    fi

    echo ""
    echo -e "${DIM}Documentation: https://github.com/z451047442-debug/swarmlo${NC}"
    echo -e "${DIM}Issues: https://github.com/z451047442-debug/swarmlo/issues${NC}"
    echo ""
}
...
setup_mcp_server() {
    ...
    if claude mcp list 2>/dev/null | grep -q "swarmlo\|claude-flow"; then
    ...
    if [ "$GLOBAL" = "1" ]; then
        claude mcp add swarmlo -e CLAUDE_FLOW_CWD="$HOME" -- swarmlo mcp start 2>/dev/null && \
            print_substep "MCP server configured ✓" || \
            print_warning "MCP setup failed - run manually: claude mcp add swarmlo -e CLAUDE_FLOW_CWD=\"\$HOME\" -- swarmlo mcp start"
    else
        claude mcp add swarmlo -e CLAUDE_FLOW_CWD="$HOME" -- npx -y swarmlo@${VERSION} mcp start 2>/dev/null && \
            print_substep "MCP server configured ✓" || \
            print_warning "MCP setup failed - run manually: claude mcp add swarmlo -e CLAUDE_FLOW_CWD=\"\$HOME\" -- npx -y swarmlo@latest mcp start"
    fi
    echo ""
}
...
run_doctor() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        swarmlo doctor 2>&1 || true
    else
        npx swarmlo@${VERSION} doctor 2>&1 || true
    fi
    echo ""
}
...
run_init() {
    ...
    if [ "$GLOBAL" = "1" ]; then
        swarmlo init --yes 2>&1 || true
    else
        npx swarmlo@${VERSION} init --yes 2>&1 || true
    fi
    echo ""
}
...
main() {
    ...
    print_success "${BOLD}Swarmlo is ready!${NC}"
    echo ""
}
