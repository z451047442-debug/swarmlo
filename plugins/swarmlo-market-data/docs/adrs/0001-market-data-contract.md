...
`swarmlo-market-data` (v0.1.0) — feed ingestion, OHLCV normalization, candlestick pattern detection. 1 agent + 2 skills + 1 command (5 subcommands).

### Three real bugs found in skills

1. `market-ingest/SKILL.md` referenced `mcp__plugin_swarmlo-core_swarmlo__embeddings_embed` — that tool name does not exist. Real tool is `embeddings_generate` (per [swarmlo-knowledge-graph ADR-0001](../../swarmlo-knowledge-graph/docs/adrs/0001-knowledge-graph-contract.md), same fix).
2. `market-ingest/SKILL.md` used `agentdb_hierarchical-store` with `namespace: 'market-data'`. Per [swarmlo-agentdb ADR-0001 §"Namespace convention"](../../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md), `agentdb_hierarchical-*` routes by **tier** (`working|episodic|semantic`), not namespace. The namespace arg was silently ignored.
3. `market-pattern/SKILL.md` had the same hierarchical-recall + namespace bug **plus** the same pattern-store + namespace bug as swarmlo-cost-tracker (`agentdb_pattern-store` with `namespace: 'market-patterns'` — ReasoningBank ignores namespace).
...
```bash
bash plugins/swarmlo-market-data/scripts/smoke.sh
# Expected: "11 passed, 0 failed"
```

## Related

- `plugins/swarmlo-cost-tracker/docs/adrs/0001-cost-tracker-contract.md` — same namespace-routing bug class
- `plugins/swarmlo-knowledge-graph/docs/adrs/0001-knowledge-graph-contract.md` — same `embeddings_embed` rename
- `plugins/swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention this ADR fixes a violation of

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/swarmlo-market-data/`. Contract elements implemented: `embeddings_embed` → `embeddings_generate` tool-name drift fixed; `agentdb_hierarchical-*` namespace arg bug fixed (switched to `memory_*`); `agentdb_pattern-store` namespace arg bug fixed; smoke-as-contract gate defined in `scripts/smoke.sh` (11 checks).
