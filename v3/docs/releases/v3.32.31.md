# Swarmlo v3.32.31: Governed Flywheel Candidates and Explicit Autopilot Scope

Swarmlo's self-improvement loop should explore useful candidates without
...
- Keep Darwin and local proposers subordinate to Swarmlo's evaluation,
...
```bash
npm install --global swarmlo@3.32.31
swarmlo doctor
```
...
```bash
swarmlo autopilot config --task-sources issues
```
...
```bash
swarmlo metaharness flywheel status
```

Run one governed evaluation using the built-in proposer:

```bash
swarmlo metaharness flywheel run --proposer local
```
