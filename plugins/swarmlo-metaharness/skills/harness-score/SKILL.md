...
Surfaces the upstream `metaharness score` CLI as a swarmlo skill. Use when
Claude Code needs to assess whether a repo is ready for harness adoption
before recommending the user run `npx swarmlo init` or `harness-mint`.
...
## Phase-0 baseline (swarmlo's own scorecard, measured 2026-06-16)
...
Swarmlo passes its own readiness check. `memoryUsefulness: 40` is the
...
```bash
node plugins/swarmlo-metaharness/scripts/score.mjs --alert-on-fit-below 70 --format json
```
...
and exits 0. Swarmlo continues to function — this is the architectural
