...
- `mcp__plugin_swarmlo-core_swarmlo__memory_store` / `memory_search` — persist and query analysis history
- `mcp__plugin_swarmlo-core_swarmlo__neural_predict` — SONA regime prediction
- `mcp__plugin_swarmlo-core_swarmlo__agentdb_pattern-search` — find similar historical patterns
...
Message schema: `RegimeVerdict` in `plugins/swarmlo-neural-trader/src/pipeline-messages.ts`.
