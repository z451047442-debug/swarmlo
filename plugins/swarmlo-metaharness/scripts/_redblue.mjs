...
//
// Workaround: install the package once into a swarmlo-owned cache dir
// (~/.swarmlo/redblue-cache-<pin>, via _invoke.ensureCachedInstall) and invoke
...
// @claude-flow/cli/package.json + swarmlo/package.json.
...
export const REDBLUE_CACHE_DIR = join(
  homedir(), '.swarmlo', `redblue-cache-${REDBLUE_PIN_VERSION.replace(/[~^]/g, '')}`,
);
