...
```bash
claude --plugin-dir plugins/swarmlo-ddd
```
...
- **Verification:** `bash plugins/swarmlo-ddd/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `ddd-patterns` AgentDB namespace (kebab-case, follows the convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/swarmlo-ddd/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmlo-ddd plugin contract](./docs/adrs/0001-ddd-contract.md)

## Related Plugins

- `swarmlo-agentdb` — namespace convention owner; backing store for the domain graph
- `swarmlo-adr` -- Document domain decisions as Architecture Decision Records
- `swarmlo-sparc` -- Architecture phase leverages DDD bounded context patterns
- `swarmlo-migrations` -- Align migration boundaries with aggregate roots
