...
# List and audit
npx swarmlo-cli@3.39.1 memory list --namespace NAMESPACE --limit 20

# Consolidated search across all namespaces
npx swarmlo-cli@3.39.1 memory search --query "QUERY" --limit 10
```
...
- **swarmlo-agentdb**: Full AgentDB backend with HNSW vector_indexes table
- **swarmlo-ruvector**: FlashAttention-3, Graph RAG, hybrid search, DiskANN
- **swarmlo-rvf**: Portable RVF format for cross-machine memory export/import
- **swarmlo-knowledge-graph**: Entity-relationship graphs over memory entries
- **swarmlo-intelligence**: SONA trajectory learning from retrieval patterns
