...
- **Verification:** `bash plugins/swarmlo-swarm/scripts/smoke.sh` is the contract.
...
This plugin owns the `swarm-state` AgentDB namespace (kebab-case, follows the convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/swarmlo-swarm/scripts/smoke.sh
# Expected: "11 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmlo-swarm plugin contract (12-tool MCP surface, anti-drift defaults, Monitor streaming, smoke as contract)](./docs/adrs/0001-swarm-contract.md)

## Related Plugins

- `swarmlo-agentdb` — namespace convention owner
- `swarmlo-autopilot` — owns the 270s cache-aware /loop heartbeat for long-running swarms
- `swarmlo-intelligence` — `hooks_route` powers swarm agent recommendation per task
