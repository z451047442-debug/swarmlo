...
```bash
claude --plugin-dir plugins/swarmlo-observability
```
...
- **Verification:** `bash plugins/swarmlo-observability/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `observability` AgentDB namespace (base-name exception per [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md), same precedent as `federation` and `migrations`). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
```bash
bash plugins/swarmlo-observability/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmlo-observability plugin contract (namespace-routing fix, smoke as contract)](./docs/adrs/0001-observability-contract.md)

## Related Plugins

- `swarmlo-agentdb` — namespace convention owner; defines the routing rules ADR-0001 fixes a violation of
- `swarmlo-cost-tracker` -- Token usage metrics feed into cost attribution
- `swarmlo-iot-cognitum` -- Reuses Z-score anomaly detection for telemetry patterns
- `swarmlo-market-data` -- Data feed health and ingestion latency monitoring
