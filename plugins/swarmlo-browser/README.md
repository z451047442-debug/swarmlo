...
Session-as-skill browser automation. Playwright-backed via 23 `mcp__plugin_swarmlo-core_swarmlo__browser_*` tools, with each session captured as a first-class **RVF cognitive container** holding manifest + trajectory + screenshots + sanitized cookies + findings, indexed in AgentDB and gated by AIDefence.
...
`/swarmlo-browser` is a verb dispatcher:

```bash
/swarmlo-browser ls [--query <text>]      # list sessions, AgentDB-indexed
/swarmlo-browser show <session-id>        # manifest + trajectory + verdict
/swarmlo-browser replay <session-id>      # re-drive trajectory
/swarmlo-browser export <session-id>      # rvf export → tar.zst
/swarmlo-browser fork <session-id>        # rvf derive → new lineage-tracked session
/swarmlo-browser purge <session-id>       # destroy, keep redacted manifest
/swarmlo-browser doctor                   # check Playwright, MCP, AgentDB, AIDefence
```
...
|-----------|-----|-------|---------|
| `browser-sessions` | `<rvf-id>` | manifest summary + verdict + tags | session index for `/swarmlo-browser ls` |
...
```bash
bash plugins/swarmlo-browser/scripts/smoke.sh
...
```bash
bash plugins/swarmlo-browser/scripts/replay-spike.sh
```
...
- [`ADR-0001` — Adopt session-as-skill architecture for swarmlo-browser](./docs/adrs/0001-browser-skills-architecture.md)

## Related Plugins

- `swarmlo-ruvector` — trajectory hooks, SONA pattern distillation, MCP tools
- `swarmlo-agentdb` — controllers backing `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies`
- `swarmlo-aidefence` — PII / prompt-injection gates
- `swarmlo-federation` — cross-installation session sharing via RVF export
