 ...
 *
 *   [1/3] STATIC ADAPTER CONTRACT — `plugins/swarmlo-neural-trader/src/sublinear-adapter.ts`
 ...
 *
 *   [2/3] STATIC SKILL CONTRACT — `plugins/swarmlo-neural-trader/skills/trader-portfolio-cg/SKILL.md`
 *         must exist with `mcp__swarmlo-sublinear__solve` in `allowed-tools`,
...
if (!existsSync(ADAPTER_TS)) {
  ...
  check(
    ...
    'native dispatch is gated by this two-probe detection (globalThis + SWARMLO_SUBLINEAR_NATIVE env var)',
  );
  check(
  ...
  check(
    'adapter honours `SWARMLO_SUBLINEAR_NATIVE` env-var override',
    /SWARMLO_SUBLINEAR_NATIVE/.test(src),
  ...
  check(
    'adapter references the MCP tool by canonical name `mcp__swarmlo-sublinear__solve`',
    /mcp__swarmlo-sublinear__solve/.test(src),
...
if (!existsSync(SKILL_MD)) {
  ...
  check(
    'skill declares `mcp__swarmlo-sublinear__solve` in allowed-tools',
    /^allowed-tools:[^\n]*mcp__swarmlo-sublinear__solve/m.test(skill),
  ...
  check(
    ...
    /^allowed-tools:[^\n]*mcp__plugin_swarmlo-core_swarmlo__memory_store/m.test(skill),
  ...
  check(
    'skill documents the `SWARMLO_NEURAL_TRADER_DISABLE_CG` disable flag',
    /SWARMLO_NEURAL_TRADER_DISABLE_CG/.test(skill),
  ...
  check(
    'skill documents the `SWARMLO_SUBLINEAR_NATIVE` env-var override',
    /SWARMLO_SUBLINEAR_NATIVE/.test(skill),
...
try {
  ...
  // determined by whether the harness has mounted `mcp__swarmlo-sublinear__solve`
  // (or `SWARMLO_SUBLINEAR_NATIVE=1` is set). The smoke validates the contract
