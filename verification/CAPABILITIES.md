...
Add capabilities by extending the `runners` map in `plugins/swarmlo-core/scripts/witness/perf.mjs`. The framework supports any synchronous benchmark that throws on failure.
...
node plugins/swarmlo-core/scripts/witness/perf.mjs

# Run with baseline comparison (median of last 5 entries per capability)
node plugins/swarmlo-core/scripts/witness/perf.mjs --baseline
...
- name: Performance verification
  run: |
    node plugins/swarmlo-core/scripts/witness/perf.mjs --baseline --json > /tmp/perf.json
    node -e "
...
- **CLI startup time** — `swarmlo --version` is the obvious metric, but currently dominated by node startup + module graph; not stable enough as a regression signal until the cli-core split (PR #1764) lands.
...
- [`plugins/swarmlo-core/scripts/witness/perf.mjs`](../plugins/swarmlo-core/scripts/witness/perf.mjs) — benchmark runner
