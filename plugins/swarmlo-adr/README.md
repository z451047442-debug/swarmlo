...
```bash
claude --plugin-dir plugins/swarmlo-adr
```
...
- **Verification:** `bash plugins/swarmlo-adr/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `adr-patterns` AgentDB namespace. It defers to [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md) for naming rules. Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.

`adr-patterns` follows kebab-case `<plugin-stem>-<intent>` per the convention. The plugin uses it for semantic ADR search and for cross-project pattern transfer (via `hooks_transfer` in `swarmlo-intelligence`).
...
- [`ADR-0001` — swarmlo-adr plugin contract (pinning, namespace coordination, smoke as contract)](./docs/adrs/0001-adr-plugin-pattern.md)
- [`ADR-0002` — Reconcile deleted ADRs (hard-delete primitive + drop-and-rebuild reindex)](./docs/adrs/0002-reconcile-deleted-adrs.md)

## Related Plugins

- `swarmlo-agentdb` — namespace convention owner; backing store for the ADR graph
- `swarmlo-ddd` — document domain decisions as ADRs
- `swarmlo-sparc` — Architecture phase (Phase 3) produces ADRs
- `swarmlo-migrations` — schema change decisions recorded as ADRs
- `swarmlo-jujutsu` — ADR-aware diff analysis on PRs
- `swarmlo-intelligence` — `hooks_transfer` ships ADR patterns across projects
