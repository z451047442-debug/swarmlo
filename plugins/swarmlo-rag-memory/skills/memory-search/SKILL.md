...
allowed-tools: Bash Read mcp__plugin_swarmlo-core_swarmlo__memory_search mcp__plugin_swarmlo-core_swarmlo__memory_store mcp__plugin_swarmlo-core_swarmlo__memory_list mcp__plugin_swarmlo-core_swarmlo__memory_retrieve mcp__plugin_swarmlo-core_swarmlo__memory_search_unified mcp__plugin_swarmlo-core_swarmlo__agentdb_pattern-search mcp__plugin_swarmlo-core_swarmlo__agentdb_context-synthesize
argument-hint: "<query> [--hybrid] [--graph-rag] [--namespace NAME]"
---

# Memory Search (SOTA)

State-of-the-art semantic search across Swarmlo memory with multiple retrieval strategies.
...
2. **Select retrieval strategy**:
   ...
   **Dense search (default)**:
   ```bash
   npx swarmlo-cli@3.39.1 memory search --query "QUERY" --namespace NAMESPACE --limit 10
   ```
   ...
   **Smart retrieval** (when --smart or complex recall needed):
   ```bash
   npx swarmlo-cli@3.39.1 memory search --query "QUERY" --smart --limit 10
   ```
   ...
   **Unified cross-namespace**:
   `mcp__plugin_swarmlo-core_swarmlo__memory_search_unified({ query: "QUERY", limit: 10 })`
...
5. **Synthesize context** (for complex queries):
   `mcp__plugin_swarmlo-core_swarmlo__agentdb_context-synthesize({ query: "QUERY", sources: ["patterns", "tasks", "solutions"] })`
