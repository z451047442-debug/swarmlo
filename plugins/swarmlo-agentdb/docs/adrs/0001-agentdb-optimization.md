...
### Today's `swarmlo-agentdb`

`swarmlo-agentdb` is a thin documentation wrapper around three MCP tool families exposed by `@claude-flow/cli`: `agentdb_*` (controller bridge), `embeddings_*` (RuVector ONNX engine), and `ruvllm_hnsw_*` (WASM-backed pattern router). The plugin ships six files:

- `.claude-plugin/plugin.json:2` — `name: "swarmlo-agentdb"`, `version: "0.2.0"`, keywords `agentdb, ruvector, hnsw, embeddings, vector-search`.
...
3. **Namespacing convention.** Every consumer plugin (`swarmlo-browser` defines `browser-sessions / browser-selectors / browser-templates / browser-cookies` in its ADR-0001 §3; `swarmlo-rag-memory` references `claude-memories / patterns / tasks / solutions`; `swarmlo-intelligence` writes to `pattern`) reinvents namespace naming. There is no contract from `swarmlo-agentdb` about how namespaces should be named, what they should contain, or how they are GC'd. The `agentdb_*` tools mostly do not even take a namespace parameter — they route to controllers (`reasoningBank`, `hierarchicalMemory`, `causalGraph`) — but the CLI fallback `memory_store` and `embeddings_search` *do* take namespace strings, so the surface is mixed and undocumented.
...
`swarmlo-agentdb` is the substrate plugin — `swarmlo-browser` (ADR-0001) explicitly depends on AgentDB namespaces and controller-managed sessions; `swarmlo-rag-memory` and `swarmlo-intelligence` route through the same controllers; the whole "memory-as-substrate" story in `CLAUDE.md` rests on it. Documentation drift in this plugin is contagious — every downstream plugin inherits the wrong tool count, the wrong controller count, and the missing quantization story. We just fixed the same class of drift in `swarmlo-ruvector` (ADR-0001 there); the same pattern applies here.

## Decision

We propose six changes to `swarmlo-agentdb`. None requires modifying AgentDB itself; all are plugin-local edits plus one new smoke script.
...
Add a new section to `README.md` titled **"Namespace convention"** that swarmlo-agentdb owns and downstream plugins consume. The contract:

- **Naming**: `<plugin-stem>-<intent>` kebab-case. Examples: `browser-sessions`, `browser-selectors`, `browser-cookies` (swarmlo-browser), `claude-memories` (swarmlo-rag-memory's bridge), `pattern` (swarmlo-intelligence ReasoningBank fallback).
...
- **GC posture**: swarmlo-agentdb does not GC namespaces. Consumer plugins that want lifecycle (e.g., `browser-sessions` after `purge`) own their own deletion via `memory_delete` + `agentdb_consolidate`. Document this so consumers don't expect cleanup we don't provide.
...
- RaBitQ goes from invisible to a documented quantization workflow — downstream plugins handling large corpora (`swarmlo-rag-memory`, future `swarmlo-knowledge-graph` integrations) get a 32× memory-reduction story by reference, not by re-discovery.
- HNSW becomes tunable instead of a magic number. Consumer plugins choose an operating point.
- A namespace convention exists. `swarmlo-browser`'s ADR §3 already implements it ad hoc; this ADR formalizes it so the next plugin author doesn't re-derive it.
...
```bash
bash plugins/swarmlo-agentdb/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

Plus three documentation invariants checked by a one-line `grep` each:

- `! grep -rn "19 AgentDB controllers\|all 19 controllers\|19 Controllers" plugins/swarmlo-agentdb/` — no occurrence remains.
- `grep -q "embeddings_rabitq_build" plugins/swarmlo-agentdb/skills/vector-search/SKILL.md` — quantization workflow is documented.
- `grep -q "Namespace convention" plugins/swarmlo-agentdb/README.md` — namespace contract section exists.

## Related

- `plugins/swarmlo-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent (this ADR mirrors §5 and §6).
- `plugins/swarmlo-browser/docs/adrs/0001-browser-skills-architecture.md` — `browser-*` namespace family that motivated §4 (namespace convention contract).
