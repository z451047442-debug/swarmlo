...
describe('fuseDenseSparse — dense + m3-native-sparse fusion (ADR-382)', () => {
  it('combines normalised dense and sparse with 1:0.3 default weights', () => {
    const dense = [0.1, 0.9, 0.5];
    const sparse = [10, 0, 5];
    const fused = fuseDenseSparse(dense, sparse);
    // dense normalises to [0, 1, 0.5]; sparse to [1, 0, 0.5].
    // index 0: 0 + 0.3*1 = 0.3; index 1: 1 + 0 = 1 → dense signal dominates.
    expect(fused[1]).toBeCloseTo(1, 6);
    expect(fused[0]).toBeCloseTo(0.3, 6);
    expect(fused[2]).toBeCloseTo(0.5 + 0.3 * 0.5, 6);
  });

  it('honours custom weights', () => {
    const fused = fuseDenseSparse([0, 1], [1, 0], 0.0, 1.0);
    expect(fused).toEqual([1, 0]); // sparse-only ordering
  });

  it('handles constant vectors without divide-by-zero', () => {
    const fused = fuseDenseSparse([0.5, 0.5], [0, 0]);
    expect(fused.every((s) => Number.isFinite(s))).toBe(true);
  });

  it('throws on mismatched lengths', () => {
    expect(() => fuseDenseSparse([0.1], [0.2, 0.3])).toThrow(/length mismatch/);
  });
});
