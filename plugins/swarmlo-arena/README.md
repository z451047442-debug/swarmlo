...
Competitive ruliology for Swarmlo swarms — **arenas**, **tournaments**, and **adaptive
co-evolution** of program strategies. Implements the first executable slice of Swarmlo
...
## MCP tools (`swarmlo-arena/mcp-tools`)
...
Full run artifacts are written to `.swarmlo/arena/<runId>.json` (exact replay). Each tool result
also carries an `agentdb` payload so the command layer can store a searchable summary via
`mcp__plugin_swarmlo-core_swarmlo__memory_store` (namespace `arena`) — the local stand-in for the RuVector data
layer (ADR-196/197), enabling queries like *"tournaments where grim dominated"*.

## Scope

v1 is intentionally Swarmlo-only and core-untouched:
