 ...
 * expiry timestamp). Swarmlo is where that envelope is *enforced* — a
 * deterministic, non-LLM gate in front of every MCP tool call and every
 * `Agent`/`Task` dispatch. ADR-380 §3 requires "Swarmlo logs every decision
 ...
 * signed-artifact family in `plugins/swarmlo-neural-trader/src/signed-*.ts`)
 ...
 *     (`plugins/swarmlo-neural-trader/src/signed-artifact.ts`,
 ...
 * @see plugins/swarmlo-neural-trader/src/signed-artifact.ts — signing/canonicalization pattern
...
export interface SignedCasaReceipt extends CasaReceiptInput {
  schema: 'swarmlo-agntcy-casa-receipt/v1';
