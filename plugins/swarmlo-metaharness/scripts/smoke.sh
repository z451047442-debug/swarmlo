#!/usr/bin/env bash
# Structural smoke test for swarmlo-metaharness v0.1.1 (ADR-150 Phase 1).
set -u
...
if [[ "$v" != "0.1.1" ]]; then
  ...
  miss=""
  for k in swarmlo metaharness harness scorecard genome mcp-scan threat-model router adr-150 adr-148 adr-149 optional-dependency graceful-degradation subprocess phase-1-mvp; do
...
F="$ROOT/commands/swarmlo-metaharness.md"
miss=""
...
step "16. swarmlo wrapper keeps metaharness out of hard dependencies (ADR-150 constraint #2)"
F="$ROOT/../../swarmlo/package.json"
...
# swarmlo to depend on metaharness at all. Historically this smoke asserted
# the presence of metaharness in swarmlo optionalDeps, which conflicted with
# the #2561 "guard" budget (SWARMLO_MAX=0 for optionalDependencies) that was
# added to protect the cold `npx -y swarmlo@alpha --version` startup path
# from a heavy optional-dep install. #2561 is stronger evidence: metaharness
# is on its FORBIDDEN list because installing it during swarmlo's postinstall
# caused a measured `npx --version` timeout. Correct reading of ADR-150 #2:
# if metaharness IS present anywhere in swarmlo's deps, it MUST be optional;
# absence is fine (swarmlo delegates to @claude-flow/cli, which itself keeps
...
" 2>/dev/null && ok || bad "metaharness leaked into swarmlo wrapper hard dependencies"
...
EXPECTED_JOBS=(
  "score"            # iter 1 — score harness against swarmlo
...
#   - `npm install swarmlo` → uses swarmlo/package.json
...
SWARMLOPJ="$ROOT/../../swarmlo/package.json"
...
for pkg in "metaharness" "@metaharness/router" "@metaharness/kernel" "@metaharness/redblue"; do
  ...
  SWARMLO_PIN=$(node -e "console.log(JSON.parse(require('fs').readFileSync('$SWARMLOPJ')).optionalDependencies?.['$pkg'] || '')" 2>/dev/null)
  ...
  if [[ -z "$ROOT_PIN" && -z "$SWARMLO_PIN" && -z "$CLI_PIN" ]]; then
    continue
  fi
  # Compare non-empty pins — they must all match.
  PINS=$(echo "$ROOT_PIN $SWARMLO_PIN $CLI_PIN" | tr ' ' '\n' | grep -v '^$' | sort -u | wc -l | tr -d ' ')
  if [[ "$PINS" != "1" ]]; then
    miss="$miss ${pkg}-pin-drift:root=${ROOT_PIN},swarmlo=${SWARMLO_PIN},cli=${CLI_PIN}"
  fi
done
[[ -z "$miss" ]] && ok || bad "$miss"
...
for pj in "$ROOT/../../package.json" "$ROOT/../../swarmlo/package.json" "$ROOT/../../v3/@claude-flow/cli/package.json"; do
  [[ -f "$pj" ]] || continue
...
#   plugins/swarmlo-metaharness/skills/harness-foo/SKILL.md
...
# SUBCOMMANDS map should appear as `npx swarmlo metaharness <X>` for
# discoverability.
DISP="$ROOT/../../v3/@claude-flow/cli/src/commands/metaharness.ts"
CMD="$ROOT/../../CLAUDE.md"
...
for k in $KEYS; do
  COUNT=$((COUNT + 1))
  grep -qE "npx swarmlo metaharness ${k}([ \\\\]|$)" "$CMD" 2>/dev/null \
...
# but forgets the map (or vice-versa), `swarmlo metaharness X` errors at
...
# Runtime: tripwire passes on swarmlo's own audit
node "$F" >/dev/null 2>&1 || miss="$miss tripwire-fails-locally"
[[ -z "$miss" ]] && ok || bad "$miss"
...
# (real swarmlo audit has 1 INFO finding; baseline file with no findings → introduced=1)
BC=$(mktemp)
...
# as a swarmlo skill or MCP tool. It clones arbitrary git URLs which is too
# powerful to expose to agents. (Upstream supports it; swarmlo deliberately
...
# static import of @metaharness/* breaks the "swarmlo works without the dep"
...
grep -q "swarmlo metaharness drift-from-history" "$CMD" 2>/dev/null || miss="$miss claude-md-no-subcommand"
...
step "17z8. swarmlo doctor checks the integration layer, not just upstream (iter 45)"
miss=""
DOC="$ROOT/../../v3/@claude-flow/cli/src/commands/doctor.ts"
...
grep -q "swarmlo metaharness similarity" "$CMD" 2>/dev/null || miss="$miss claude-md-no-subcommand"
...
grep -q "Swarmlo remains operational if every MetaHarness package is removed" "$F" || miss="$miss no-constraint-quote"
# All 4 rules documented
grep -q "no-metaharness-smoke.yml" "$F" || miss="$miss no-ci-gate-ref"
# Command surface + tool surface enumerated
grep -q "npx swarmlo metaharness score" "$F" || miss="$miss no-cli-example"
# audit-allow: standalone-mcp-prefix — this checks the repository CLAUDE.md, not a plugin skill.
...
grep -q "swarmlo metaharness oia-audit" "$HOOKS" 2>/dev/null || miss="$miss hooks-tip-missing"
[[ -z "$miss" ]] && ok || bad "$miss"
