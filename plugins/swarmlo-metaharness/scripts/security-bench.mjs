...
//
// CONNECTION TO SWARMLO ADR-155
// ===========================
// swarmlo's ADR-155 (#2417) proposes a nightly self-learning security harness
...
// gives us the empirical baseline that swarmlo's Phase 2 loop A needs before
...
function safetyChecks() {
  if (ARGS.population < 1 || ARGS.population > 20) {
    console.error('security-bench: --population must be 1..20 (swarmlo cap)');
    process.exit(2);
  }
  if (ARGS.cycles < 1 || ARGS.cycles > 100) {
    console.error('security-bench: --cycles must be 1..100 (swarmlo cap)');
    process.exit(2);
  }
}
