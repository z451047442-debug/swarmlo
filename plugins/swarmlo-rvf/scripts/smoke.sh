...
step "6. RVF cross-reference to swarmlo-browser sessions"
F="$ROOT/README.md"
grep -q "swarmlo-browser" "$F" \
  && grep -qE "session.*RVF|RVF.*session" "$F" \
  && ok || bad "browser-RVF cross-reference missing"

step "7. RVF tooling cross-reference to swarmlo-ruvector"
F="$ROOT/README.md"
grep -q "swarmlo-ruvector" "$F" \
