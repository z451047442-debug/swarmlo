 ...
 *
 * Two env vars steer the shim (see ../scripts/swarmlo-hook.cjs) at the build
 * under test instead of whatever's on the runner's PATH:
 *   - CLAUDE_PLUGIN_ROOT       — resolves swarmlo-hook.cjs's own path (real
 *                                per-hook-invocation env var, always set)
 *   - SWARMLO_HOOK_CLI_OVERRIDE  — bypasses the swarmlo/claude-flow/npx PATH
 ...
 *   node plugins/swarmlo-core/scripts/test-hooks.mjs <path-to-cli-binary>
 *
...
//   - npx fallthrough:     "npx --yes swarmlo-cli@3.39.1"
...
if (!cliInvoke) {
  ...
  console.error('  node test-hooks.mjs "npx --yes swarmlo-cli@3.39.1"');
  process.exit(2);
}
...
const run = (name, cmd, stdin, assertions, options = {}) => {
  const env = {
    ...
    SWARMLO_HOOK_CLI_OVERRIDE: options.cliOverride ?? cliInvoke,
    SWARMLO_HOOK_SKIP_NPX: '1',
    ...options.env,
  };
  for (const key of options.unsetEnv ?? []) delete env[key];
  if (options.debug !== false) env.SWARMLO_HOOK_DEBUG_STDOUT = '1';
  else delete env.SWARMLO_HOOK_DEBUG_STDOUT;
...
{
  const dedupDir = mkdtempSync(join(tmpdir(), 'swarmlo-hook-dedup-test-'));
  const payload = '{"tool_use_id":"toolu_dedup_2640","tool_input":{"file_path":"/tmp/dedup.ts"}}';
  const env = {
    ...process.env,
    CLAUDE_PLUGIN_ROOT: PLUGIN_ROOT,
    CLAUDE_PROJECT_DIR: PLUGIN_ROOT,
    SWARMLO_HOOK_CLI_OVERRIDE: `${process.execPath} ${HOOK_RECORDER}`,
    SWARMLO_HOOK_DEBUG_STDOUT: '1',
    SWARMLO_HOOK_SKIP_NPX: '1',
    SWARMLO_HOOK_DEDUP_DIR: dedupDir,
  };
  try {
