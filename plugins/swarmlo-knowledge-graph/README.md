...
```bash
claude --plugin-dir plugins/swarmlo-knowledge-graph
```
...
- **Verification:** `bash plugins/swarmlo-knowledge-graph/scripts/smoke.sh` is the contract.

## Namespace coordination

This plugin owns the `knowledge-graph` AgentDB namespace (kebab-case, follows the convention from [swarmlo-agentdb ADR-0001 §"Namespace convention"](../swarmlo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
## G7 controllers (activated in swarmlo 3.6.23+ / 3.6.24)
...
```bash
bash plugins/swarmlo-knowledge-graph/scripts/smoke.sh
# Expected: "10 passed, 0 failed"
```

## Architecture Decisions

- [`ADR-0001` — swarmlo-knowledge-graph plugin contract (embeddings_generate fix, namespace coordination, smoke as contract)](./docs/adrs/0001-knowledge-graph-contract.md)

## Related Plugins

- `swarmlo-agentdb` -- The G7 controllers above ship via this plugin's runtime; install both for full graph + traversal coverage; namespace convention owner
- `swarmlo-ruvector` -- HNSW indexing for fast semantic search across graph nodes
- `swarmlo-adr` -- ADR dependency graphs share the same causal edge model
