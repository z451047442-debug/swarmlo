...
The substrate plugin for Swarmlo memory. Wraps three CLI MCP families — `agentdb_*` (controller bridge, 15 tools), `embeddings_*` (RuVector ONNX engine, 10 tools), and `ruvllm_hnsw_*` (WASM-backed pattern router, 3 tools) — into discoverable skills and commands. Other plugins (`swarmlo-browser`, `swarmlo-rag-memory`, `swarmlo-intelligence`) compose this substrate; this plugin owns the namespace convention and the smoke contract for the substrate as a whole.
...
/plugin install swarmlo-agentdb@swarmlo
```
...
- **Verification:** the bundled smoke script is the source of truth (`bash plugins/swarmlo-agentdb/scripts/smoke.sh`). If smoke passes against your CLI version, the plugin's contract holds.
...
`graphAdapter` is currently disabled pending an external graph-DB connection (tracked in ADR-095). Other Level-2/3 security controllers (`mutationGuard`, `attestationLog`, `gnnService`, `rvfOptimizer`, `guardedVectorBackend`) were activated by ADR-095 G7 in swarmlo 3.6.23+.
...
| Plugin | Namespaces |
|---|---|
| `swarmlo-browser` | `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies` |
| `swarmlo-rag-memory` | (uses bridge target `claude-memories`) |
| `swarmlo-intelligence` | (uses fallback target `pattern`) |
...
| `trajectory-begin/step/end` (ruvector hooks) | ruvector substrate (separate plugin) | sona/agentdb namespaces handled by `swarmlo-ruvector` | See `plugins/swarmlo-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` |
...
- [`ADR-0001` — Optimize swarmlo-agentdb (accurate surface, RaBitQ, namespacing, smoke contract)](./docs/adrs/0001-agentdb-optimization.md)

## Related Plugins

- `swarmlo-rag-memory` — simple store/search/recall interface; consumes the `claude-memories` reserved namespace
- `swarmlo-intelligence` — SONA neural patterns; consumes the `pattern` reserved namespace via ReasoningBank
- `swarmlo-browser` — composes the namespace convention for `browser-sessions/-selectors/-templates/-cookies` (ADR-0001 §3 there)
- `swarmlo-ruvector` — pinned ruvector CLI; sibling substrate plugin
