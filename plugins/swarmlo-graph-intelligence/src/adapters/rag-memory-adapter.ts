 ...
 *
 * `swarmlo-rag-memory` ships Graph-RAG multi-hop retrieval. This adapter
...
export function ragMemoryGraphId(namespace?: string): string {
  return namespace
    ? `swarmlo-rag-memory:chunks:${namespace}`
    : 'swarmlo-rag-memory:chunks:default';
}

export class RagMemoryAdapter implements SublinearAdapter {
  readonly graphId: string;
  readonly ownerPlugin = 'swarmlo-rag-memory';
