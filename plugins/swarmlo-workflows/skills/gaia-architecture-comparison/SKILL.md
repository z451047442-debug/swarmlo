...
description: Side-by-side comparison of swarmlo vs HAL vs other GAIA harnesses — capability gaps, design decisions, and improvement roadmap
argument-hint: "[--focus=tools|routing|memory|cost]"
allowed-tools: Bash Read mcp__plugin_swarmlo-core_swarmlo__memory_search mcp__plugin_swarmlo-core_swarmlo__memory_store
---

# GAIA Architecture Comparison Skill

Compare swarmlo's GAIA benchmark harness against the Princeton HAL reference
...
| Dimension | swarmlo | HAL reference | Gap |
|-----------|-------|--------------|-----|
...
| Self-consistency | voting.ts (Track A) | Not in reference | swarmlo advantage |
| Hardness routing | predictor.ts (Track Q) | Not in reference | swarmlo advantage |
| Memory | AgentDB HNSW | None | swarmlo advantage |
