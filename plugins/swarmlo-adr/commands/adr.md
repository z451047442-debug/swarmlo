...
3. Store in AgentDB with `mcp__plugin_swarmlo-core_swarmlo__agentdb_hierarchical-store`: key `mem:ADR-NNN`, tier `semantic`, and a JSON-encoded string value
...
3. Create causal edge: `mcp__plugin_swarmlo-core_swarmlo__agentdb_causal-edge` from old to new with relation "supersedes"
...
1. Query `mcp__plugin_swarmlo-core_swarmlo__agentdb_causal-query` for all ADR relationships
...
1. Call `mcp__plugin_swarmlo-core_swarmlo__memory_search` with the query in namespace `adr-patterns`
