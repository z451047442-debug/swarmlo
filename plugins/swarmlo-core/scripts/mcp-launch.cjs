 ...
 * back to `npx -y swarmlo-cli@3.39.1 mcp start` only when no local
 * install resolves.
 *
 * Without this, plugins/swarmlo-core/.mcp.json's `npx -y
 * swarmlo-cli@3.39.1` always re-resolved to whatever was newest on
...
function resolveLocalCliBin(cwd, home) {
  const candidates = [
    join(home, '.claude', 'plugins', 'marketplaces', 'swarmlo', 'bin', 'cli.js'),
    join(cwd, 'node_modules', '@claude-flow', 'cli', 'bin', 'cli.js'),
    join(cwd, 'node_modules', 'swarmlo', 'bin', 'cli.js'),
...
function buildLaunchSpec(localBin) {
  ...
  return {
    command: npxCmd,
    args: ['-y', 'swarmlo-cli@3.39.1', ...MCP_ARGS],
