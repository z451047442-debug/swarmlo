 ...
 *   2. Always exits 0 (even when swarmlo binary is not installed)
...
const SHIM_PATH = resolve(REPO_ROOT, 'plugins', 'swarmlo-core', 'scripts', 'swarmlo-hook.cjs');
// Every distributable mirror must keep its platform pair on one dist-tag.
const SHIM_PAIRS = [
  resolve(REPO_ROOT, 'plugins', 'swarmlo-core', 'scripts'),
  resolve(REPO_ROOT, 'plugin', 'scripts'),
  resolve(REPO_ROOT, '.claude-plugin', 'scripts'),
].map((directory) => [
  resolve(directory, 'swarmlo-hook.sh'),
  resolve(directory, 'swarmlo-hook.cjs'),
]);
...
function runShim(subcommand, extraArgs = [], stdinInput = '') {
  return spawnSync(
    ...
    [SHIM_PATH, subcommand, ...extraArgs],
    {
      ...
      env: { ...process.env, SWARMLO_HOOK_SKIP_NPX: '1' },
    }
  );
}

console.log(`Testing swarmlo-hook.cjs on ${process.platform}`);
...
// Test 10: dist-tag parity — .cjs shims + .sh shim must reference the same swarmlo@<tag>
// (Regression guard for #2600 — .cjs drifted to swarmlo@latest while .sh used swarmlo@alpha.)
{
  const tagRe = /swarmlo@([a-z0-9][a-z0-9._-]*)/i;
  const extractTag = (p) => {
...
console.log('ok: smoke-swarmlo-hook-cjs passed');
