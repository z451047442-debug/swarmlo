...
//     a temp dir under /tmp/swarmlo-mint-<timestamp>/ if not given — never the
//     project root (swarmlo behavioral rule "never save to root folder").
...
//     confirmation" pattern in swarmlo's CLAUDE.md.
//
// USAGE
...
function safetyChecks() {
  ...
  if (!ARGS.target) {
    ARGS.target = resolve(tmpdir(), `swarmlo-mint-${Date.now()}-${ARGS.name}`);
  }
